...
dxTerrainY::dxTerrainY (dSpaceID space, dReal *pHeights,dReal vLength,int nNumNodesPerSide, int bFinite, int bPlaceable, int jumpStep ) :
dxGeom (space,bPlaceable),m_pHolesMap(0)
{
    ...
    if( jumpStep < nNumNodesPerSide )
        m_nJumpStep = nNumNodesPerSide+1;
    else
        m_nJumpStep = jumpStep;

	m_bFinite = bFinite;

    dReal* ptr(pHeights);

	for (int i=0;i<(nNumNodesPerSide+1) * (nNumNodesPerSide+1);i++,++ptr)
	{

        if( i )
        {
            if( !(i % (nNumNodesPerSide+1)) )
            {
                ptr+= ((m_nJumpStep - (nNumNodesPerSide+1)));
            }
        }
...
inline dReal dxTerrainY::GetHeight(int x,int z)
{
    return m_pHeights[ (z * m_nJumpStep ) + x ];
}
...
dGeomID dCreateTerrainY(dSpaceID space, dReal *pHeights,dReal vLength,int nNumNodesPerSide, int bFinite, int bPlaceable, int step)
{
	return new dxTerrainY(space, pHeights,vLength,nNumNodesPerSide,bFinite,bPlaceable,step);
}

void dSetTerrainYHoleIn( dGeomID g, int x, int z )
{
...
bool dGetTerrainYHoleIn( dGeomID g, int x, int z )
{
	dUASSERT (g && g->type == dTerrainYClass,"argument not a terrain");
	dxTerrainY *t = (dxTerrainY*) g;
    return t->IsHole( x, z );
...
bool dTerrainYHasHoles( dGeomID g )
{
	dUASSERT (g && g->type == dTerrainYClass,"argument not a terrain");
	dxTerrainY *t = (dxTerrainY*) g;
    return t->HasHole();
...
void dTerrainYRemoveHoles( dGeomID g )
{
	dUASSERT (g && g->type == dTerrainYClass,"argument not a terrain");
	dxTerrainY *t = (dxTerrainY*) g;
    t->RemoveHole();
...
dReal dGeomTerrainYPointDepthInt( dGeomID g, int x, int y, int z)
{
	dUASSERT (g && g->type == dTerrainYClass,"argument not a terrain");
	dxTerrainY *t = (dxTerrainY*) g;
	return t->GetHeight(x,z) - y;
...
void dGeomTerrainYUpdateAABB( dGeomID g )
{
	dUASSERT (g && g->type == dTerrainYClass,"argument not a terrain");
	dxTerrainY *t = (dxTerrainY*) g;
	return t->UpdateAll();
...
#endif		
        // @optimize static ray (not a new one) and reuse
        int nA[3],nB[3];
...
int dTestRayAgainstAABB( dReal* aabb, dxRay *ray ) 
{
    dReal aabbCenterPoint[3] = {  // c
        (aabb[0] + aabb[1]) * dReal(.5),
        (aabb[2] + aabb[3]) * dReal(.5),
        (aabb[4] + aabb[5]) * dReal(.5)
    };    

    dReal halfLengthExtens[3] = { // e
        aabb[1] - aabbCenterPoint[0],
        aabb[3] - aabbCenterPoint[1],
        aabb[5] - aabbCenterPoint[2]
    };
   
    dReal segmentPointA[3] = {
        ray->pos[0],
        ray->pos[1],
        ray->pos[2]
    };

    dReal segmentPointB[3] = {
        segmentPointA[0] + ray->length * ray->R[0*4+2],
        segmentPointA[1] + ray->length * ray->R[1*4+2],
        segmentPointA[2] + ray->length * ray->R[2*4+2]
    };

    dReal segmentMidPoint[3] = { // m
        (segmentPointA[0] + segmentPointB[0]) * dReal(.5),
        (segmentPointA[1] + segmentPointB[1]) * dReal(.5),
        (segmentPointA[2] + segmentPointB[2]) * dReal(.5)
    };

    dReal segmentHalfLength[3] = { // d
        segmentPointB[0] - segmentMidPoint[0],
        segmentPointB[1] - segmentMidPoint[1],
        segmentPointB[2] - segmentMidPoint[2]
    };

    segmentMidPoint[0] -= aabbCenterPoint[0];
    segmentMidPoint[1] -= aabbCenterPoint[1];
    segmentMidPoint[2] -= aabbCenterPoint[2];

    dReal adx( fabs( segmentHalfLength[0] ) );

    if( fabs( segmentMidPoint[0] ) > halfLengthExtens[0] + adx )
        return 0;

    dReal ady( fabs( segmentHalfLength[1] ) );

    if( fabs( segmentMidPoint[1] ) > halfLengthExtens[1] + ady )
        return 0;

    dReal adz( fabs( segmentHalfLength[2] ) );

    if( fabs( segmentMidPoint[2] ) > halfLengthExtens[2] + adz )
        return 0;
    ...
    if( fabs(segmentMidPoint[1]*segmentHalfLength[2] - segmentMidPoint[2]*segmentHalfLength[1]) > 
        halfLengthExtens[1] * adz + halfLengthExtens[2] * ady )
        return 0;

    if( fabs(segmentMidPoint[2]*segmentHalfLength[0] - segmentMidPoint[0]*segmentHalfLength[2]) > 
        halfLengthExtens[0] * adz + halfLengthExtens[2] * adx )
        return 0;

    if( fabs(segmentMidPoint[0]*segmentHalfLength[1] - segmentMidPoint[1]*segmentHalfLength[0]) > 
        halfLengthExtens[0] * ady + halfLengthExtens[1] * adx )
        return 0;
...
int dCollideTerrainY(dxGeom *o1, dxGeom *o2, int flags,dContactGeom *contact, int skip)
{
    ...
    {
        if( !dTestRayAgainstAABB( o1->aabb, (dxRay*)(o2) ) )
        {
            return 0;
        }
    }
    ...
    /// Checking if the terrain has a hole/s
    if( terrain->HasHole() )
    {
	    for (i=nMinX;i<nMaxX;i++)
	    {
		    ...
		    {
                if( !terrain->IsHole(i,j) ||
                    !terrain->IsHole(i+1,j) || 
                    !terrain->IsHole(i,j+1) 
                    /*terrain->IsHole(i+1,j+1)*/ )
                {
...
void dxTerrainY::UpdateAll()
{

    // computes the new max and mins
    dReal* ptr(m_pHeights);

	for (int i=0;i<(m_nNumNodesPerSide+1) * (m_nNumNodesPerSide+1);i++,++ptr)
	{

        if( i )
        {
            if( !(i % (m_nNumNodesPerSide+1)) )
            {
                ptr+= ((m_nJumpStep - (m_nNumNodesPerSide+1)));
            }
        }
...
bool dxTerrainY::HasHole() const
{
    return m_pHolesMap != 0;
}

void dxTerrainY::SetHole(int x,int z)
{
    if( !m_pHolesMap )
    {
        /// computing buffer size
        int nodes(m_nNumNodesPerSide/8);

        /// assumes that the nodes numbers are 2 powered to n plus one
        ++nodes;

        nodes *= (m_nNumNodesPerSide+1);

        /// creating memory bits buffer
        m_pHolesMap = new char[nodes];

        /// no holes at all
        memset( m_pHolesMap, 0, nodes );
    }
    ...
    int lenght((m_nNumNodesPerSide>>3)+1);

    m_pHolesMap[ z * lenght + qx ] |= 1 << rx;
}

bool dxTerrainY::IsHole(int x,int z) const
{
    ...
    return m_pHolesMap[ z * lenght + qx ] & (1 << rx);
}

void dxTerrainY::RemoveHole()
{
    delete [] m_pHolesMap;
