...
// Simple class to manage destruction of allocated bitmap.
class wxRBitmapManager
{
public:
    wxRBitmapManager() { m_bmp = (wxBitmap*) NULL; }
    ~wxRBitmapManager() { if ( m_bmp ) delete m_bmp; }
    
    wxBitmap* SetXpm ( const char* xpm_data[] )
    {
        m_bmp = new wxBitmap( (const char**)xpm_data );
        return m_bmp;
    }
    wxBitmap* m_bmp;
};
...
static wxRBitmapManager gs_bmp_gtk_arrow;
...
bool wxCustomTextCtrlHandler::OnKeyEvent ( wxKeyEvent& event )
{
    ...
    else if ( event.m_eventType == wxEVT_CHAR &&
...
bool wxCustomTextCtrlHandler::OnMouseEvent ( wxMouseEvent& event )
{
    ...
    if ( event.m_eventType == wxEVT_MOTION )
    {
    ...
    else if ( event.m_eventType == wxEVT_LEFT_DOWN 
#if !wxPG_TEXTCTRL_DOUBLE_CLICK_MODE
              || event.m_eventType == wxEVT_LEFT_DCLICK
#endif
            )
    {
...
#if wxPG_TEXTCTRL_DOUBLE_CLICK_MODE
    else if ( event.m_eventType == wxEVT_LEFT_DCLICK )
    {
...
bool wxCCustomButtonHandler::OnMouseEvent ( wxMouseEvent& event )
{
    ...
    wxCustomControlManager* manager = ctrl->m_manager;
    if ( event.m_eventType == wxEVT_LEFT_DOWN || event.m_eventType == wxEVT_LEFT_DCLICK ||
         ( event.m_eventType == wxEVT_ENTER_WINDOW && manager->IsDragging() ) )
    {
        if ( !m_down )
        {
            m_down = 1;
    ...
    else if ( event.m_eventType == wxEVT_LEFT_UP ||
         ( event.m_eventType == wxEVT_LEAVE_WINDOW && manager->IsDragging() ) )
    {
        if ( m_down )
        {
            m_down = 0;
            ...
            if ( event.m_eventType == wxEVT_LEFT_UP && m_label.length() )
            {
    ...
    else if ( event.m_eventType == wxEVT_ENTER_WINDOW ||
              event.m_eventType == wxEVT_LEAVE_WINDOW
            )
    {
        DRAW_CTRL_IN_DATA_FUNC(manager)
    }
...
bool wxCustomControlManager::ProcessMouseEvent ( wxMouseEvent& event )
{
    ...
    if ( mouse_focused )
    {
        ...
        {
            int old_type = event.m_eventType;
            event.m_eventType = wxEVT_ENTER_WINDOW;
        #if __MOUSE_DEBUGGING__
            ...
            event.m_eventType = old_type;
            m_flags |= wxCCM_FL_MOUSE_INSIDE;
        }

        // Dragging management
        if ( event.m_eventType == wxEVT_LEFT_DOWN )
        {
            StartDragging();
    ...
    if ( event.m_eventType == wxEVT_LEFT_UP )
        StopDragging();
...
void wxCustomControlManager::DoMouseLeave ( wxMouseEvent& event, wxCustomControlHandler* newdata, bool force )
{
    if ( m_dataMouseFocus )
    {
        int evt_type = event.m_eventType;
        ...
        if ( m_flags & wxCCM_FL_MOUSE_INSIDE ) 
        {
            ...
            m_flags &= ~(wxCCM_FL_MOUSE_INSIDE);
            event.m_eventType = evt_type; // must repair the event
        }
...
{
    /*wxLogDebug(wxT("char=%i"),event.m_eventType);
