...
// * Enable OptimizeSplitterPos?
//   Problem: Currently easy to do only for current page. Result is basicly
//     worse than without.
//   Solution: Somehow relay need to do optimizesplitterpos to
//     manager? Maybe additional bit to iFlags?
// * If wxParentProperty is loaded from a text source, its value should
//   be set *after* all children are inserted.
// * Move vspacing fix to stable as well.
// * Enable font images?
// * Fix the bug: CorrectEditorWidgetSize doesn't work when vertical
//     size increases (only, not hori) and scrollbar disappears.
//     NOTE: This may be already fixed?
// Here's entry to changelog:
//- If scrollbar was visible and in scrolled position, and there was editor
//  control, then resizing the grid vertically (for instance, by manually
//  adjusting description box) so that the scrollbar would disappear skewed
//  the editor position.
// * After 1.0.x compatibility removed, drop itemcount argument support.
//
// High Priority but Difficult:
...
// * Resolve global variable locking (caused deadlock under Linux)
...
void wxPGChoices::SetId( size_t id )
{
    wxASSERT ( IsOk() );

    if ( !IsTemporary() )
    {
        wxPGHashMapChoices* socs =
            (wxPGHashMapChoices*) wxPGGlobalVars->m_dictChoices;
...
void wxPropertyGrid::DumpAllocatedChoiceSets()
{
    ...
    wxPGHashMapChoices* socs = (wxPGHashMapChoices*) wxPGGlobalVars->m_dictChoices;

    wxLogDebug(wxT("****************************************************************"));

    wxPGHashMapChoices::iterator soc_it;
    int soc_count = 0;
...
wxPGChoices* wxPropertyGrid::CreateChoicesArray(const wxChar** labels,
                                                const long* values,
                                                int itemcount)
{
    wxASSERT(labels);

    wxPGChoices* choices = wxPGChoices::GetOrCreate( (size_t) labels );

    // Fill it only if was created
    if ( !choices->GetCount() )
        choices->Set(labels,values,itemcount);
...
wxPGChoices* wxPropertyGrid::CreateChoicesArray(const wxArrayString& labels,
                                                ...
                                                bool acceptLabelsAsId)
{
    ...
    wxPGChoices* choices = wxPGChoices::GetOrCreate( id );

    // Fill it only if was created
    if ( !choices->GetCount() )
        choices->Set(labels,values);
...
wxPGChoices* wxPropertyGrid::CreateChoicesArray( wxPGChoices& origChoices )
{
    wxASSERT( origChoices.IsOk() );

    wxPGChoices* choices = wxPGChoices::GetOrCreate(origChoices.GetId());

    // Fill it only if was created
    if ( !choices->GetCount() )
        choices->Set(origChoices.GetLabels(),origChoices.GetValues());

    origChoices.SetId(choices->GetId());
...
// If there are choices with given id, returns it. Otherwise creates a new
// one.
wxPGChoices* wxPGChoices::GetOrCreate(size_t id)
{
    //wxLogDebug(wxT("wxPGChoices::GetOrCreate(%u)"),(unsigned int)id);
    ...
    if ( id )
    {
        wxPGHashMapChoices::iterator it;
        it = socs->find(id);

        if ( it != socs->end() )
        {
            //wxLogDebug(wxT("  \\--> found"));

            wxPGChoices* soc = it->second;

            // Don't check if its temporary - zero refcounts should be allowed
            // (populator uses them that way, atleast)
    ...
    wxPGChoices* soc = new wxPGChoices();
    if (!id)
        id = (size_t) &soc->m_arrLabels;

    //wxLogDebug(wxT("  \\--> created(%u)"),(unsigned int)id);
...
// Leave this in wxPropertyContainerMethods instead of wxPGChoices so
// that users can use it more intuitively.
wxPGChoices* wxPropertyContainerMethods::GetChoicesArray(size_t id)
{
    wxPGHashMapChoices* socs =
        (wxPGHashMapChoices*) wxPGGlobalVars->m_dictChoices;
    ...
    return (wxPGChoices*) NULL;
}

// -----------------------------------------------------------------------

void wxPropertyContainerMethods::SetPropertyChoices(wxPGId id, wxPGChoices& choices)
{
    ...
    if ( ci.m_choices )
    {
        wxPGChoices* cons = *ci.m_choices;
        wxPGUnRefChoices( cons );

        *ci.m_choices = wxPropertyGrid::CreateChoicesArray(choices);

        // This may be needed to trigger some initialization
        p->DoSetValue(p->GetValueType()->GetDefaultValue());
    }
}
                                                   ...
                                                   int value )
{
    ...
    wxPGChoices& cons = GetPropertyChoices(id);
...
wxPGGlobalVarsClass::wxPGGlobalVarsClass()
{
    ...
    //m_dictChoices = new wxPGHashMapChoices();

    //m_emptyChoicesData.m_refCount = 0xFFFFFFF;
...
wxPGGlobalVarsClass::~wxPGGlobalVarsClass()
{
    ...
    //wxPGHashMapChoices* socs = (wxPGHashMapChoices*) m_dictChoices;

    //delete socs;
    // Do *not* place wxPGChoices destructors beyond this point
    // (they need m_dictChoices)

    // Destroy empty choices data
    //delete g_wxPG_emptyChoicesData;
}
...
wxPGProperty::~wxPGProperty()
{
    //if ( m_label == wxT("Label") && GetParent()->GetLabel() == wxT("Appearance") )
    //    wxMessageBox(wxT("Label being deleted"));
...
long wxPGProperty::GetValueAsLong() {

    return this->GetGrid()->GetPropertyValueAsLong( GetId() );
}
...
size_t wxPGProperty::SetChoices ( size_t id, const wxArrayString& labels, const wxArrayInt& values )
{
    ...
    if ( ci.m_choices )
    {
        wxPGChoices::UnRef(*ci.m_choices);

        // Create new with given id
        wxPGChoices* choices = wxPGChoices::GetOrCreate(id);

        // Add items if it was empty
        if ( !choices->GetCount() && &labels )
        {
            choices->SetLabels(labels);
            if ( &values && values.GetCount() )
                choices->SetValues(values);
        }

        wxASSERT ( choices->GetId() );

        *ci.m_choices = choices;
...
//#ifdef __WXDEBUG__
//static int gs_allocatedDataExtensions = 0;
//#endif
        ...
        const wxPoint& pos, const wxSize& sz, wxWindow** ) const
{
    ...
    int odcb_flags = wxCB_READONLY|wxNO_BORDER;
...
void wxPGChoiceEditor::UpdateControl ( wxPGProperty* property, wxWindow* ctrl ) const
{
    wxASSERT ( ctrl );
#if wxPG_NAT_CHOICE_BORDER_ANY
    wxOwnerDrawnComboBox* cb = (wxOwnerDrawnComboBox*)((wxPGClipperWindow*)ctrl)->GetControl();
#else
...
//#ifdef __WXDEBUG__
//    wxLogDebug(wxT("%i properties had data extension"),gs_allocatedDataExtensions);
//#endif
}
...
# endif
    ...
    // Check if we are really unfocused
    // (wxPG_FL_FOCUSED cannot be trusted)
    /*int reallyFocused = true;
    if ( !(m_iFlags & wxPG_FL_FOCUSED) )
    {
        wxWindow* focused = wxWindow::FindFocus();
        if ( focused != this &&
             (!m_wndPrimary || focused != m_wndPrimary) &&
             (!m_wndSecondary || focused != m_wndSecondary) )
        {
            reallyFocused = false;
        }
    }*/
...
/*
int wxPropertyGrid::GetOptimalSplitterPos(wxClientDC& dc,
                                          wxPGPropertyWithChildren* pwc,
                                          bool subProps)
{
    size_t i;
    ...
    for ( i=0; i<pwc->GetCount(); i++ )
    {
        ...
        if ( p->GetParentingType() <= 0 )
        {
            ...
            w += m_marginwidth + ( ((int)p->m_depth) * m_subgroup_extramargin );
        ...
        {
            w = GetOptimalSplitterPos( dc, (wxPGPropertyWithChildren*) p, subProps );
...
void wxPropertyGrid::OptimizeSplitterPosition( bool subProps )
{
    wxClientDC dc(this);
    dc.SetFont(m_font);

    int maxW = GetOptimalSplitterPos(dc, m_pState->m_properties, subProps );
...
void wxPropertyGrid::CorrectEditorWidgetSizeX ( int newSplitterx, int newWidth )
{
    ...
    if ( m_wndPrimary )
    {
        ...
        //wxLogDebug(wxT("modulus: %i"),(int)(r.y % wxPG_PIXELS_PER_UNIT));
...
void wxPropertyGrid::SetupEventHandling ( wxWindow* argWnd, int id )
{
    ...
    if ( argWnd == m_wndPrimary )
    {
        ...
        CONNECT_CHILD(wxEVT_LEFT_DCLICK,(wxMouseEventFunction),OnMouseLeftDoubleClick)
    }
...
void wxPropertyGrid::OnResize ( wxSizeEvent& event )
{
    ...
    if ( width!=old_width )
    {
              ...
              (!sb_vis_toggled||!(m_iFlags & wxPG_FL_GOOD_SIZE_SET))) )
        {
            ...
            //else
            //    OptimizeSplitterPosition( false );
        }
    ...
    /*
    // No need for this anymore since RecalculateVirtualSize is always called.
...
bool wxPropertyGrid::HandleMouseLeftDoubleClick( int, unsigned int y, wxMouseEvent& )
{
    if ( y < m_bottomy )
    {
        // Select property here as well
        ...
        // Send right click event.
        wxPropertyGridEvent evt( wxEVT_PG_LEFT_DCLICK, GetId() );
        evt.SetPropertyGrid (this);
        evt.SetEventObject (this);
        evt.SetProperty (p);
        GetEventHandler()->AddPendingEvent(evt);
...
void wxPropertyGrid::OnMouseLeftDoubleClick( wxMouseEvent &event )
{
    //wxLogDebug(wxT("wxPropertyGrid::OnMouseRightClickChild"));
    int x,y;
    wxASSERT ( m_wndPrimary );
...
void wxPropertyGrid::OnFocusEvent ( wxFocusEvent& event )
{
    ...
    {
        //wxLogDebug(wxT("wxPropertyGrid::OnFocusEvent(wxEVT_SET_FOCUS,%s)"),((wxWindow*)event.GetEventObject())->GetName().c_str());

        m_iFlags |= wxPG_FL_FOCUSED;
    }
    else
    {
        //wxLogDebug(wxT("wxPropertyGrid::OnFocusEvent(wxEVT_KILL_FOCUS,%s)"),((wxWindow*)event.GetEventObject())->GetName().c_str());
         ...
         (oldFlags & wxPG_FL_FOCUSED) )
    {
        ...
        if ( !(m_iFlags & wxPG_FL_FOCUSED) )
        {
            //wxLogDebug(wxT(" \\--> pg focus truly killed"));
        ...
        {
            //wxLogDebug(wxT(" \\--> pg focus truly set"));
...
DEFINE_EVENT_TYPE( wxEVT_PG_LEFT_DCLICK )
DEFINE_EVENT_TYPE( wxEVT_PG_CREATECTRL )
DEFINE_EVENT_TYPE( wxEVT_PG_DESTROYCTRL )
...
wxPGId wxPropertyContainerMethods::DoGetPropertyByName ( wxPGNameStr name ) const
{
...
wxPGId wxPropertyContainerMethods::GetPropertyByName( wxPGNameStr name,
                                                      wxPGNameStr subname ) const
{
                                         ...
                                         const wxArrayInt& choiceValues)
{
    ...
    if ( p )
    {
        ...
        if ( choicesId )
        {
            ...
            //wxPGChoices& rchs = m_propGrid->GetPropertyChoices(p);

            //wxLogDebug(wxT("  choices_id: %u (count=%i)"),choicesId,rchs.GetCount());
...
WX_PG_IMPLEMENT_STRING_CALLBACK_PROPERTY(wxStringCallBackProperty)

bool wxStringCallBackPropertyClass::OnButtonClick( wxPropertyGrid* /*propgrid*/, wxString& value ) {
    
    if ( dialog_->ShowModal() == wxID_OK )
    {
        value = dialog_->GetTitle();
        return true;
    }
    return false;
}

void wxStringCallBackPropertyClass::SetCallBackDialog( wxObject& dialog ) {
    dialog_ = static_cast<wxDialog*>(&dialog);
}


void wxPropertyGrid::Refresh()
{
    if( m_selected )
    {
        m_selected->UpdateControl(m_wndPrimary);
...
void wxPropertyGrid::MakeIntegerSpinControl( const wxString& name ) {
    wxPGRegisterEditorClass ( SpinCtrl );

    SetPropertyEditor( name, wxPG_EDITOR(SpinCtrl) );
}

void wxPropertyGrid::MakeIntegerSpinControlById( const wxPGId id ) {
    wxPGRegisterEditorClass ( SpinCtrl );
...
wxPGProperty* wxAdvImageFileProperty ( const wxString& label, const wxString& name )
{
    return new wxAdvImageFilePropertyClass (label,name,wxEmptyString);
}
...
////////////////////
// -----------------------------------------------------------------------
class wxPGAnyCtrlEditor : public wxPGTextCtrlEditor
{
    WX_PG_DECLARE_EDITOR_CLASS()
public:
    virtual ~wxPGAnyCtrlEditor();

    // See below for short explanations of what these are suppposed to do.
    virtual wxWindow* CreateControls ( wxPropertyGrid* propgrid, wxPGProperty* property,
        const wxPoint& pos, const wxSize& sz, wxWindow** psecondary ) const;
    virtual void UpdateControl ( wxPGProperty* property, wxWindow* wnd ) const;
    virtual bool wxPGAnyCtrlEditor::OnEvent ( wxPropertyGrid* propgrid, wxPGProperty* property,
        wxWindow* wnd, wxEvent& event ) const;
    virtual bool CopyValueFromControl ( wxPGProperty* property, wxWindow* wnd ) const;
    virtual void SetValueToUnspecified ( wxWindow* wnd ) const;
    virtual void SetControlStringValue ( wxWindow* wnd, const wxString& txt ) const;
...
// This macro also defined global wxPGEditor_SpinCtrl for storing
// the singleton class instance.
WX_PG_IMPLEMENT_EDITOR_CLASS(AnyCtrl,wxPGAnyCtrlEditor,wxPGEditor)


// Trivial destructor.
wxPGAnyCtrlEditor::~wxPGAnyCtrlEditor() { }

// Create controls and initialize event handling.
wxWindow* wxPGAnyCtrlEditor::CreateControls ( wxPropertyGrid* propgrid, wxPGProperty* property,
        const wxPoint& pos, const wxSize& sz, wxWindow** ) const
{
    // Get initial value (may be none if value is 'unspecified')
    wxString text;
    ...
    wxSlider* copy( new wxSlider );

    copy->Create( propgrid, wxPG_SUBID1, 0, 0, 100 );

    propgrid->SetSizeControl( sz );
    propgrid->SetPositionControl( pos );

    wxPropertyGridEvent evt( wxEVT_PG_CREATECTRL, propgrid->GetId() );
    evt.SetPropertyGrid (propgrid);
    evt.SetEventObject (propgrid);
    evt.SetProperty (property);

    propgrid->ProcessEvent( evt );

    propgrid->Connect( wxPG_SUBID1, wxEVT_SIZE,
                       (wxObjectEventFunction) (wxEventFunction) (wxCommandEventFunction)
                       &wxPropertyGrid::OnCustomEditorEvent );
...
// Copies value from property to control
void wxPGAnyCtrlEditor::UpdateControl ( wxPGProperty*, wxWindow* ) const
{
}

// Control's events are redirected here
bool wxPGAnyCtrlEditor::OnEvent ( wxPropertyGrid* propgrid, wxPGProperty* property,
    wxWindow* wnd, wxEvent& event ) const
{
    return wxPGTextCtrlEditor::OnEvent(propgrid,property,wnd,event);
}
...
// Copies value from control to property
bool wxPGAnyCtrlEditor::CopyValueFromControl ( wxPGProperty* property, wxWindow* /*wnd*/ ) const
{
    return property->SetValueFromInt(1,wxPG_FULL_VALUE);
}

// Makes control look like it has unspecified value
void wxPGAnyCtrlEditor::SetValueToUnspecified ( wxWindow* ) const
{
}
...
// (obviously, not all controls can implement this properly,
//  but wxSpinCtrl can)
void wxPGAnyCtrlEditor::SetControlStringValue ( wxWindow*, const wxString& ) const
{
}

void wxPGAnyCtrlEditor::OnFocus ( wxPGProperty*, wxWindow* ) const
{
}

void wxPropertyGrid::MakeAnyControl( const wxString& name)
{
    wxPGRegisterEditorClass ( AnyCtrl );

    SetPropertyEditor( name, wxPG_EDITOR(AnyCtrl) );
}

void wxPropertyGrid::MakeAnyControlById( const wxPGId id )
{
    wxPGRegisterEditorClass ( AnyCtrl );

    SetPropertyEditor( id, wxPG_EDITOR(AnyCtrl) );
}
...
void wxPropertyGrid::SelectProperty( wxPGProperty* p, unsigned int flags )
{
    ...
    if ( m_frozen )
    {
        ...
        if ( m_wndSecondary )
        {
            if( m_wndSecondary->IsKindOf(CLASSINFO(wxSlider)) )
            {
                wxPropertyGridEvent evt( wxEVT_PG_DESTROYCTRL, GetId() );
                evt.SetPropertyGrid (this);
                evt.SetEventObject (this);
        ...
        if ( m_wndPrimary )
        {
            if( m_wndPrimary->IsKindOf(CLASSINFO(wxSlider)) )
            {
                wxPropertyGridEvent evt( wxEVT_PG_DESTROYCTRL, GetId() );
                evt.SetPropertyGrid (this);
                evt.SetEventObject (this);
