...
public:
    ...
    /// show an assert message (may block the program until the user choose action)
    virtual int Assert(const char* msg, va_list argList);
    /// get absolute path
    virtual nString GetAbsolutePath();
