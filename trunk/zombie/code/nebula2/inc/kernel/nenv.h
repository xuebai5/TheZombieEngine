...
public:
    ...
    void SetO(nRoot *o);
    /// Sets the value of a pointer (use only in runtime, not save)
    void SetP(void * p);
...
inline
void 
nEnv::SetL(nArg* _l, int len)
{
    this->arg.SetL(_l, len);
}
...
inline
int 
nEnv::GetL(nArg*& args) const
{
    return this->arg.GetL(args);
}
