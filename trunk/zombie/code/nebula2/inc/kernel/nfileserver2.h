...
public:
    ...
    /// sets a path alias
    bool SetAssign(const char * assignName, const nString & pathName);
    /// sets a path alias group
    bool SetAssignGroup(const char * assignName, const nArray<nString> & paths);
    /// gets a path alias
    const char* GetAssign(const char * assignName)const;
    /// gets a path alias group
    bool GetAssignGroup(const char* assignName, nArray<nString> & assigns)const;
    /// is the name specified an assign group
    bool IsAssignGroup(const char * assignName)const;
    /// Reset assign repository to default values.
    void ResetAssigns();
    /// Expand all path
    void GetAllManglePath(const char* path, nArray<nString> &assigns);
    /// expand path alias to real path
    nString ManglePath(const nString& pathName, bool read = true);
    /// make path components
    virtual bool MakePath(const nString& path);
    /// copy a file
    virtual bool CopyFile(const nString& from, const nString& to);
    /// copy a file
    virtual bool MoveFile(const nString& from, const nString& to);
    /// delete a file
    virtual bool DeleteFile(const nString& filename, bool read = true);
    /// delete an empty directory
    virtual bool DeleteDirectory(const nString& dirName, bool read = true);
    /// delete a directory, even if it is not empty (handle with care!)
    virtual bool DeleteDirectoryRecursive(const nString& dirName, bool read = true);
    ...
    /// get hidden status of a file
    bool IsFileHidden(const nString& filename);
    ...
    virtual bool FileExists(const nString& pathName);
    /// check if directory exists
    virtual bool DirectoryExists(const nString& pathName) const;
    /// get all the file names of a directory
    void GetAllFilesFrom( const char * pathName, nArray<nString> & files );
    ...
    /// find a file recursively from a base path
    bool FindFile(const char * basePath, const char * fileName, nString & path);

#ifndef NGAME
    /// contract real  path to alias to path only if it is posible.
    bool UnManglePath(nString& pathName, nString assignName);
    /// copy the contents of one directory to another
    bool CopyDirectory(const nString& sourceDirPath, const nString& destinationDirPath);
    /// copy the contents of one directory to another recursively
    bool CopyDirectoryRecursively(const nString& sourceDirPath, const nString& destinationDirPath);
#endif

protected:
    /// cleanup a path name in place
    void CleanupPathName(nString& str)const;
...
private:
    ...
    void InitTempAssign();
    /// Expand all path
    void GetAllManglePathRecursively (const char* path, nArray<nString> &assigns);

#ifndef NGAME
    /// copy the contents of one directory to another
    bool CopyDirectory(const nString& sourceDirPath, const nString& destinationDirPath, bool recursive);
#endif
