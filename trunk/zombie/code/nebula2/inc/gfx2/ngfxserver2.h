...
public:
    ...
    enum TransformType
    {
        ...
        Texture2,
        Texture3,
        Light,                  ///< the current light's matrix in world space (read/write)
        LightProjection,        ///< the current light's projection matrix (read/write)
        ModelLight,
        InvModelLight,
        ModelLightProjection,   ///< the current model * light * lightProjection matrix
        ShadowProjection,
    ...
    /// set that current nebula get the mouse cursor focus
    ...
    /// get current vertex buffer mesh
    nMesh2* GetVerticesMesh() const;
    /// get current index buffer mesh
    nMesh2* GetIndicesMesh() const;
    ...
    virtual void OnUnloadMesh(nMesh2 * mesh);
    /// set current shader
    ...
    //@{
    /// 
    NSIGNAL_DECLARE('BROL', void, RestoreOnLost, 0, (), 0, ());
    //@}
    ...
    enum RenderOption
    {
        ...
        // Add new options before this line
        NumRenderOptions
    };
    ...
    void SetBoolRenderOption( RenderOption, bool );
    /// get the value of a boolean render option
    bool GetBoolRenderOption( RenderOption ) const;
    /// set the value of a float render option
    void SetFloatRenderOption( RenderOption, float );
    /// get the value of a float render option
    float GetFloatRenderOption( RenderOption ) const;
    /// set the value of an integer render option
    void SetIntRenderOption( RenderOption, int );
    /// get the value of an integer render option
    int GetIntRenderOption( RenderOption ) const;
...
protected:
    ...
    bool dirtyTransform[NumTransformTypes];
    ...
    struct RenderState
    {
        bool boolRenderOptions[ NumRenderOptions ];
        float floatRenderOptions[ NumRenderOptions ];
        int intRenderOptions[ NumRenderOptions ];
        RenderState() {}
        ~RenderState() {}
    };
    nArray< RenderState > renderStateStack;
...
//------------------------------------------------------------------------------
/**
    Get the current vertex buffer mesh.

    @return             pointer to current nMesh2 vertex buffer object
*/
inline
nMesh2*
nGfxServer2::GetVerticesMesh() const
{
    return this->refVbMesh.get_unsafe();
}

//------------------------------------------------------------------------------
/**
    Get the current index buffer mesh.

    @return             pointer to current nMesh2 index buffer object
*/
inline
nMesh2*
nGfxServer2::GetIndicesMesh() const
{
    return this->refIbMesh.get_unsafe();
}
...
inline
void
nGfxServer2::SetFocus( bool value )
{
    this->isFocus = value;
}
...
inline
void
nGfxServer2::SetCursorPosition( int /*x*/, int /*y*/ )
{
    // empty
}
...
inline
bool
nGfxServer2::InBeginFrame() const
{
