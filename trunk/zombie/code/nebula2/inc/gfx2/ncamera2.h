...
public:
    ...
    /// set custom matrix for projection
    void SetCustomMatrix( matrix44 & m );
    /// set projection type
    void SetType(Type t);
    /// get projection type
    Type GetType() const;
    ...
    /// Set fov type
    void SetFovType(TypeFov typeFov);
    /// Get fov type
    TypeFov GetFovType() const;

private:
    /// update the internal projection and inverse projection matrices
    void UpdateProjInvProj();
    /// calculate vertical angle view
    void UpdateVerticalAngleOfView();
...
nCamera2::nCamera2() :
    ...
    angleOfViewVertical(angleOfView),
...
nCamera2::SetPerspective(float aov, float aspect, float nearp, float farp)
{
    ...
    this->projDirty = true;
    this->boxDirty = true;
    this->UpdateVerticalAngleOfView();
}
...
inline
void
nCamera2::SetCustomMatrix( matrix44 & m )
{
    this->type = CustomMatrix;
    this->proj = m;
    this->projDirty = true;
    this->boxDirty = true;
}
...
{
    if ( this->type != CustomMatrix )
    {
        maxy = this->nearPlane * n_tan(this->angleOfViewVertical * 0.5f);
        miny = -maxy;
    ...
    {
        matrix44 invProjTemp;
        if ( this->projDirty )
        {
            invProjTemp = this->proj;
            invProjTemp.invert();
        }
        else
        {
            invProjTemp = this->invProj;
        }

        vector3 n( -1.0f, -1.0f, 1.0f );
        n = invProjTemp * n;
        minx = n.x;
        miny = n.y;
        minz = n.z;

        n = vector3( 1.0f, 1.0f, 1.0f );
        n = invProjTemp * n;
        maxx = n.x;
        maxy = n.y;
...
nCamera2::UpdateProjInvProj()
{
    ...
    {
    case Perspective:
        this->proj.perspFovRh(this->angleOfViewVertical, this->aspectRatio, this->nearPlane, this->farPlane);
    ...
    case CustomMatrix:
        // (assumed that it's not used for drawing)
        this->shadowProj = this->proj;
        break;
...
nCamera2::GetBox()
{
    if (this->boxDirty)
    {
        ...
        {
        case Perspective:
            tanAov = float(tan(angleOfViewVertical * 0.5f));
...
//------------------------------------------------------------------------------
/**
    Set vertical or horizontal FOV
*/
inline 
void 
nCamera2::SetFovType(TypeFov typeFov)
{
    this->typeFov = typeFov;
    this->projDirty = true;
    this->boxDirty = true;
}
//------------------------------------------------------------------------------
/**
    Get vertical or horizontal FOV
*/
inline
nCamera2::TypeFov
nCamera2::GetFovType() const
{
    return this->typeFov;
}
//------------------------------------------------------------------------------
inline 
void
nCamera2::UpdateVerticalAngleOfView()
{
    if ( Hor == this->typeFov)
    {
        this->angleOfViewVertical = 2.0f*atanf(tanf( this->angleOfView * 0.5f) / this->aspectRatio );
    } else
    {
        this->angleOfViewVertical = this->angleOfView;
    }
}
