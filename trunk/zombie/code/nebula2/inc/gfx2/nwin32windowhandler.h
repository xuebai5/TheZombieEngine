...
public:
    ...
    /// called when WM_SETFOCUS is received
    virtual bool OnSetFocus();
    /// called when WM_KILLFOCUS is received
...
{
    nAutoRef<nEnv> hwnd("/sys/env/hwnd");

    if( hwnd->GetType() == nArg::Int )
        return reinterpret_cast<HWND>( size_t(hwnd->GetI()) );
    else
        return static_cast<HWND>( hwnd->GetP() );
}
