...
public:
    ...
    void lerp(const polar2& p0, float lerpVal);
...
inline
void
polar2::lerp(const polar2& p0, float lerpVal)
{
    this->theta = p0.theta + ((this->theta - p0.theta) * lerpVal);
    this->rho   = p0.rho   + ((this->rho   - p0.rho  ) * lerpVal);
}
