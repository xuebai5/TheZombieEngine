...
public:
    ...
    /// compute the point p at which the three planes intersect (if at all)
    bool intersect(const plane& p1, const plane& p2, vector3 &p) const;

    /// get the D component given a point in the plane
    void calculate_d(const vector3& point);
...
/**
    Compute the point p at which the three planes intersect (if at all)
    (Taken from "Real time collision detection", page 213)
*/
inline
bool
plane::intersect(const plane& p1, const plane& p2, vector3 &p) const
{
    vector3 u(p1.normal() * p2.normal());
    float denom(this->normal() % u);
    const float tol(1e-06f);
    if ( n_abs(denom) < tol )
    {
        return false;
    ...
    {
        p = ( (u * (-this->d)) + (this->normal() * ( (p2.normal() * p1.d) - (p1.normal() * p2.d))) ) / denom;
        return true;
    }
}
...
//------------------------------------------------------------------------------
/**
    Calculate the D component given a point in the plane
*/
inline
void
plane::calculate_d(const vector3& point)
{
    this->d =  -((this->a * point.x) + 
                 (this->b * point.y) +
                 (this->c * point.z));
}
