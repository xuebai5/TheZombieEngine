...
public:
    ...
    /// extend the box
    void extend(const cylinder &cyl);
    ...
    /// Return if is a valid bbbox
    bool IsValid() const;
    /// check if this box completely contains the parameter box
    bool contains(const bbox3& box) const;
    /// check if this box completely contains the parameter box in 2D (XZ plane)
    bool containsXZ(const bbox3& box) const;
    /// check if this box completely contains the parameter box in 2D (XY plane)
    bool containsXY(const bbox3& box) const;
    /// check if this box completely contains the parameter box in 2D (YZ plane)
    bool containsYZ(const bbox3& box) const;
    /// return true if this box contains the position
    bool contains(const vector3& pos) const;
    /// return a bounding rectangle of the projected bounding box
    rectangle cliprect(const matrix44& viewProjection) const;
    /// return a bounding rectangle of the projected bounding box in screen coordinates,
    /// but only the part that is inside 'rect' if 'useRect' is true.
    rectangle cliprect(const matrix44& viewProjection, const rectangle &rect, bool useRect) const;
    ...
    /// check for intersection with projection volume and a rectangle
    ClipStatus clipstatus(const matrix44& viewProjection, const rectangle& rect, bool useRect);
...
//------------------------------------------------------------------------------
/**
    extend the box with the given cylinder (conservative approximation, the bbox
    will be slightly bigger than the cylinder)
*/
inline
void 
bbox3::extend(const cylinder &cyl)
{
    float extVal;

    // extend x
    if ( cyl.p1.x > cyl.p2.x )
    {
        extVal = cyl.p1.x + cyl.r;
        if ( extVal > this->vmax.x )
        {
            this->vmax.x = extVal;
        }

        extVal = cyl.p2.x - cyl.r;
        if ( extVal < this->vmin.x )
        {
            this->vmin.x = extVal;
        }
    }
    ...
    // extend y
    if ( cyl.p1.y > cyl.p2.y )
    {
        extVal = cyl.p1.y + cyl.r;
        if ( extVal > this->vmax.y )
        {
            this->vmax.y = extVal;
        }

        extVal = cyl.p2.y - cyl.r;
        if ( extVal < this->vmin.y )
        {
            this->vmin.y = extVal;
        }
    }
    ...
    // extend z
    if ( cyl.p1.z > cyl.p2.z )
    {
        extVal = cyl.p1.z + cyl.r;
        if ( extVal > this->vmax.z )
        {
            this->vmax.z = extVal;
        }

        extVal = cyl.p2.z - cyl.r;
        if ( extVal < this->vmin.z )
        {
            this->vmin.z = extVal;
        }
    }
...
inline
bool 
bbox3::containsXZ(const bbox3& box) const
{
    if ((this->vmin.x < box.vmin.x) && (this->vmax.x >= box.vmax.x) &&
        (this->vmin.z < box.vmin.z) && (this->vmax.z >= box.vmax.z))
    {
...
inline
bool 
bbox3::containsXY(const bbox3& box) const
{
    if ((this->vmin.x < box.vmin.x) && (this->vmax.x >= box.vmax.x) &&
        (this->vmin.y < box.vmin.y) && (this->vmax.y >= box.vmax.y))
    {
...
inline
bool 
bbox3::containsYZ(const bbox3& box) const
{
    if ((this->vmin.y < box.vmin.y) && (this->vmax.y >= box.vmax.y) &&
        (this->vmin.z < box.vmin.z) && (this->vmax.z >= box.vmax.z))
    {
        return true;
    }
...
//------------------------------------------------------------------------------
/**
    Returns a bounding rectangle of the projected bounding box in screen coordinates.
*/
inline
rectangle
bbox3::cliprect(const matrix44& viewProjection) const
{
    int i;
    static vector4 v0;
    static vector4 v1;
    ...
    for (i = 0; i < 8; i++)
    {
        ...
        rect.extend(v1.x / v1.w, v1.y / v1.w);
    }
...
inline
rectangle
bbox3::cliprect(const matrix44& viewProjection, const rectangle &rect, bool useRect) const
{
    int i;
    static vector4 v0;
    static vector4 v1;
    ...
    if (useRect)
    {
        for (i = 0; i < 8; i++)
        {
            ...
            newRect.extend((v1.x / v1.w), (v1.y / v1.w));
        }
    }
    else
    {
        this->cliprect(viewProjection);
    }
...
/**
    Check for intersection with a view volume defined by a view-projection
    matrix and a rectangle.
    If 'useRect' is true, 'rect' will be used to perform the clip test.
*/
inline
bbox3::ClipStatus
bbox3::clipstatus(const matrix44& viewProjection, const rectangle& rect, bool useRect)
{
    int andFlags = 0xffff;
    int orFlags  = 0;
    int i;
    ...
    if (useRect )
    {
        for (i = 0; i < 8; i++)
        {
            ...
            if (v1.x < (rect.v0.x * v1.w))       clip |= ClipLeft;
            else if (v1.x > (rect.v1.x * v1.w))  clip |= ClipRight;
            if (v1.y < (rect.v0.y * v1.w))       clip |= ClipBottom;
            else if (v1.y > (rect.v1.y * v1.w))  clip |= ClipTop;
