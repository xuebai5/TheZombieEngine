...
public:
    ...
    /// create a window of given class name and with a given name
    nGuiWindow* NewWindow(const char* className, bool visible);
    /// create a window of given class name
    nGuiWindow* NewWindowNamed(const char* className, bool visible, const char* name);
    /// create a new skin
    ...
    /// draw a brush of the current skin with a given colour
    void DrawBrush(const rectangle& r, nGuiBrush& brush, vector4 color);
    ...
    /// check if a textentry is being edit at the moment
    bool IsTextEntrySelected() const;
    /// set global active text entry
    void SetActiveTextEntry(bool f);
    /// get global state for active text entry
    bool GetActiveTextEntry() const;
    ...
    // pointer to layer, initialized when server is created
    nGuiLayer * Layer;
...
private:
    ...
    /// @todo this method is private in /vendor, don't understand why.
public:
    virtual void Render();
private:
    ...
    static const nTime toolTipFadeInTime;
    static const nTime toolTipActivationTime;
