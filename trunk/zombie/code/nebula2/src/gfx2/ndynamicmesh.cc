                         ...
                         bool shared)
{
    ...
    if (!this->refMesh.isvalid())
    {
        ...
        if (shared)
        {
            char resName[128];
            strcpy(resName, "dyn_");
            int charIndex = static_cast<int>( strlen(resName) );
            if (indexed)                                resName[charIndex++] = '0';
...
//------------------------------------------------------------------------------
/**
    Deinitialize the dynamic mesh, releasing resources if needed.
*/
bool
nDynamicMesh::Deinitialize()
{
    if (this->refMesh.isvalid())
    {
...
//------------------------------------------------------------------------------
/**
    Return the vertex components available of the dynamic mesh.
*/
int 
nDynamicMesh::GetVertexComponents() const
{
    if (this->refMesh.isvalid())
    {
        return this->refMesh->GetVertexComponents();
    }
...
//------------------------------------------------------------------------------
/**
    Returns true if the dynamic mesh has all the vertex components specified.

    @param comp [in] bitmask with all the vertex componennts to check
*/
bool 
nDynamicMesh::HasAllVertexComponents(int comp) const
{
    if (this->refMesh.isvalid())
    {
        return this->refMesh->HasAllVertexComponents(comp);
    }
                           ...
                           int& maxNumIndices)
{
    ...
    // this is not needed in BeginIndexed, left this commented intentionally
