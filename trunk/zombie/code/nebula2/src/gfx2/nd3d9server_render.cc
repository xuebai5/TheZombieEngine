...
{
#if 0
    // <FIXME> need to switch to fullscreen viewport when drawing lines
...
nD3D9Server::ClearLights()
{
    ...
    if (FFP == this->lightingType)
    {
        ...
        {
            N_IFDEF_ASSERTS(HRESULT hr = )this->d3d9Device->LightEnable(i, FALSE);
...
nD3D9Server::ClearLight(int index)
{
    ...
    {
        N_IFDEF_ASSERTS(HRESULT hr = )this->d3d9Device->LightEnable(index, FALSE);
        n_assert(SUCCEEDED(hr));
    }
}
    ...
    if (this->refSharedShader.isvalid())
    {
        ...
        switch (type)
        {
            case Model:
                if (!mvpOnly)
                {
                    this->updSharedTransform[Model] = true;
                    this->updSharedTransform[InvModel] = true;
                    this->updSharedTransform[ModelView] = true;
                    this->updSharedTransform[InvModelView] = true;
                }
                this->updSharedTransform[ModelViewProjection] = true;
                break;
            
            case View:
                if (!mvpOnly)
                {
                    this->updSharedTransform[View] = true;
                    this->updSharedTransform[InvView] = true;
                    this->updSharedTransform[ModelView] = true;
                    this->updSharedTransform[InvModelView] = true;
                }
                this->updSharedTransform[ModelViewProjection] = true;
            ...
            case Projection:
                if (!mvpOnly)
                {
                    this->updSharedTransform[Projection] = true;
                }
                this->updSharedTransform[ModelViewProjection] = true;
                break;
            ...
            case Texture0:
                if (!mvpOnly)
                {
                    this->updSharedTransform[Texture0] = true;
                }
                break;
            ...
            case Texture1:
                if (!mvpOnly)
                {
                    this->updSharedTransform[Texture1] = true;
                }
                break;
            ...
            case Texture2:
                if (!mvpOnly)
                {
                    this->updSharedTransform[Texture2] = true;
                }
                break;
            ...
            case Texture3:
                if (!mvpOnly)
                {
                    this->updSharedTransform[Texture3] = true;
                }
                break;
            case Light:
                break;
...
//------------------------------------------------------------------------------
/**
    Updates shared shader parameters for transforms
    that are used in current shader and have been updated.
    This is called from all Draw*** methods.
*/
void
nD3D9Server::UpdateSharedTransforms(nShader2* curShader)
{
    // update the shared shader parameters
    if (this->refSharedShader.isvalid())
    {
        ...
        {
            if (this->updSharedTransform[i])
            {
                switch (i)
                {
                case Model:
                    if (!curShader->IsParameterUsed(nShaderState::Model))
                    {
                        continue;
                    }
                    shd->SetMatrix(nShaderState::Model, this->GetTransform(Model));
                    break;

                case InvModel:
                    if (!curShader->IsParameterUsed(nShaderState::InvModel))
                    {
                        continue;
                    }
                    shd->SetMatrix(nShaderState::InvModel, this->GetTransform(InvModel));
                    break;

                case View:
                    if (!curShader->IsParameterUsed(nShaderState::View))
                    {
                        continue;
                    }
                    shd->SetMatrix(nShaderState::View, this->GetTransform(View));
                    break;

                case InvView:
                    if (!curShader->IsParameterUsed(nShaderState::InvView) &&
                        !curShader->IsParameterUsed(nShaderState::EyePos))
                    {
                        continue;
                    }
                    shd->SetMatrix(nShaderState::InvView, this->GetTransform(InvView));
                    shd->SetVector3(nShaderState::EyePos, this->GetTransform(InvView).pos_component());
                    break;

                case ModelView:
                    if (!curShader->IsParameterUsed(nShaderState::ModelView))
                    {
                        continue;
                    }
                    shd->SetMatrix(nShaderState::ModelView, this->GetTransform(ModelView));
                    break;

                case InvModelView:
                    if (!curShader->IsParameterUsed(nShaderState::InvModelView) &&
                        !curShader->IsParameterUsed(nShaderState::ModelEyePos))
                    {
                        continue;
                    }
                    shd->SetMatrix(nShaderState::InvModelView, this->GetTransform(InvModelView));
                    shd->SetVector3(nShaderState::ModelEyePos, this->GetTransform(InvModelView).pos_component());
                    break;

                case Projection:
                    if (!curShader->IsParameterUsed(nShaderState::Projection))
                    {
                        continue;
                    }
                    shd->SetMatrix(nShaderState::Projection, this->GetTransform(Projection));
                    break;

                case ModelViewProjection:
                    if (!curShader->IsParameterUsed(nShaderState::ModelViewProjection))
                    {
                        continue;
                    }
                    shd->SetMatrix(nShaderState::ModelViewProjection, this->GetTransform(ModelViewProjection));
                    break;

                case Texture0:
                    shd->SetMatrix(nShaderState::TextureTransform0, this->GetTransform(Texture0));
                    break;

                case Texture1:
                    shd->SetMatrix(nShaderState::TextureTransform1, this->GetTransform(Texture1));
                    break;

                case Texture2:
                    shd->SetMatrix(nShaderState::TextureTransform2, this->GetTransform(Texture2));
                    break;

                case Texture3:
                    shd->SetMatrix(nShaderState::TextureTransform3, this->GetTransform(Texture3));
                    break;
                }
            }

            this->updSharedTransform[i] = false;
        }
    }
}
...
nD3D9Server::EndScene()
{
    ...
    n_assert(this->d3d9Device);
    n_dxverify2(this->d3d9Device->EndScene(), "EndScene() on D3D device failed!");
...
nD3D9Server::Clear(int bufferTypes, float red, float green, float blue, float alpha, float z, int stencil)
{
    ...
    // no stencil buffer
    n_dxverify2(
        this->d3d9Device->Clear(0, NULL, flags, d3dColor, z, stencil),
        "In nD3D9Server::Clear(): Clear() on D3D device failed!" );
}
...
nD3D9Server::SetMesh(nMesh2* vbMesh, nMesh2* ibMesh)
{
    ...
    {    
        if ((this->refVbMesh.get_unsafe() != vbMesh) || (this->refIbMesh.get_unsafe() != ibMesh))
        {
            ...
            if (this->curVertexComponents != vbMesh->GetVertexComponents())
            {
                this->curVertexComponents = vbMesh->GetVertexComponents();

                hr = this->d3d9Device->SetVertexDeclaration(d3dVDecl);
                n_dxtrace(hr, "SetVertexDeclaration() on D3D device failed!");

                #if __NEBULA_STATS__
                ++this->statsNumVertexDeclarationChanges;
                #endif
            }
...
nD3D9Server::DrawIndexed(PrimitiveType primType)
{
    ...
    // update shared transform parameters is used in current shader
    this->UpdateSharedTransforms(shader);
...
nD3D9Server::DrawIndexedInstancedNS(PrimitiveType primType)
{
    ...
    // update shared transform parameters is used in current shader
    this->UpdateSharedTransforms(curShader);
        ...
        for (curCompIndex = 0; curCompIndex < numComps; curCompIndex++)
        {
            ...
            switch (comp.GetType())
            {
                ...
                case nShaderState::Matrix44:
                    ...
                    const matrix44& val = instStream->ReadMatrix44();
                    curShader->SetMatrix(param, val);
                    break;
            }
        }
    ...
    instStream->Unlock();
    //curShader->FlushParameterSlots(); // TODO
...
*/
void
nD3D9Server::SetRenderState()
{
    n_assert( this->d3d9Device );
    n_assert( this->d3dxLine );

    n_dxverify2(
        this->d3d9Device->SetRenderState( D3DRS_ZENABLE, this->GetBoolRenderOption(ZTestEnable) ),
        "IDirect3DDevice9::SetRenderState() failed!" );
    n_dxverify2(
        this->d3d9Device->SetRenderState( D3DRS_ZWRITEENABLE, this->GetBoolRenderOption(ZWriteEnable) ),
        "IDirect3DDevice9::SetRenderState() failed!" );
    n_dxverify2(
        this->d3dxLine->SetWidth( this->GetFloatRenderOption(LineWidth) ),
        "ID3DXLine::SetWidth() failed!" );

    int zfunc( 0 );
    switch ( this->GetIntRenderOption(ZTestFunc) )
    {
        case LessEqual:
    ...
    n_dxverify2(
        this->d3d9Device->SetRenderState( D3DRS_ZFUNC, zfunc ),
        "IDirect3DDevice9::SetRenderState() failed!" );
}
...
nD3D9Server::Flush()
{
    if (this->queryEvent == 0 || !this->statsEnabled)
    {
        return;
    }

    this->queryEvent->Issue(D3DISSUE_END);

    while (S_FALSE == this->queryEvent->GetData( NULL, 0, D3DGETDATA_FLUSH ))
    {
        // empty
    }
}
