...
*/
void
nD3D9Server::BeginShapes(nShader2* altShader)
{
    nGfxServer2::BeginShapes(altShader);

    nShapeServer* shapeServer = nShapeServer::Instance();
    if (altShader)
    {
        shapeServer->SetShader(altShader);
    }
    else
    {
        shapeServer->SetShader(this->refShapeShader.get());
    }
...
nD3D9Server::DrawShape(ShapeType type, const matrix44& model, const vector4& color)
{
    ...
    if ( type == Cone )
    {
        static bool messageShowedOnce = false;
        if ( !messageShowedOnce )
        {
            n_message( "Sorry, cone shape not implemented in nGfxServer2 yet, use nShapeServer instead" );
    ...
    nShapeServer* shapeServer = nShapeServer::Instance();
    if (shapeServer->GetShapeMesh(type))
    {
        shapeServer->DrawShape(type, model, color);
    }
    else
    {
        // update color in shader
        nShader2* shd = shapeServer->GetShader();
        ...
        this->UpdateSharedTransforms(shd);
        shd->BeginPass(0);
        n_dxverify2(
            this->shapeMeshes[type]->DrawSubset(0),
            "DrawSubset() failed in nD3D9Server::DrawShape()" );
        shd->EndPass();
...
nD3D9Server::DrawShapeNS(ShapeType type, const matrix44& model)
{
    ...
    N_IFDEF_ASSERTS(HRESULT hr = )this->shapeMeshes[type]->DrawSubset(0);
...
nD3D9Server::DrawShapePrimitives(PrimitiveType type, int numPrimitives, const vector3* vertexList, int vertexWidth, const matrix44& model, const vector4& color)
{
    ...
    // update color in shader
    nShader2* shd = this->shapeServer.GetShader();//this->refShapeShader;
    if (shd->IsParameterUsed(nShaderState::MatDiffuse))
    {
        shd->SetVector4(nShaderState::MatDiffuse, color);
    }
    ...
    this->UpdateSharedTransforms(shd);
    shd->BeginPass(0);
    N_IFDEF_ASSERTS(HRESULT hr = )this->d3d9Device->DrawPrimitiveUP(d3dPrimType, numPrimitives, vertexList, vertexWidth * 4);
                                        ...
                                        const vector4& color)
{
    ...
    D3DFORMAT d3dIndexType = D3DFMT_INDEX16;
    switch (indexType)
    {
    ...
    N_IFDEF_ASSERTS(HRESULT hr = )
                 this->d3d9Device->DrawIndexedPrimitiveUP(d3dPrimType,      // PrimitiveType
                                                          0,                // MinVertexIndex
