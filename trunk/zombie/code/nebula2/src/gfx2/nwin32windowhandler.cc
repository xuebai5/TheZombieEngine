...
nWin32WindowHandler::nWin32WindowHandler() :
    ...
    parentHwnd(0),
    hAccel(0),
    windowedStyle(WS_CLIPSIBLINGS | WS_CLIPCHILDREN | WS_POPUP | WS_CAPTION | WS_SYSMENU | WS_THICKFRAME | WS_MINIMIZEBOX | WS_VISIBLE),
...
nWin32WindowHandler::OpenWindow()
{
    ...
    nAutoRef<nEnv> nhwnd("/sys/env/hwnd");
    ...
    {        
        ...
        if (parent_hwnd.isvalid()) 
        {
            ...
            this->parentHwnd = static_cast<HWND>( parent_hwnd->GetP() );

            // we are a child, so set dimension from parent
            RECT r;
            GetClientRect(this->parentHwnd, &r);
            this->displayMode.SetWidth( static_cast<ushort>( r.right - r.left ) );
            this->displayMode.SetHeight( static_cast<ushort>( r.bottom - r.top ) );
            this->displayMode.SetType(nDisplayMode2::ChildWindow);
        } 
        else
        {
        ...
        SetWindowLong(this->hWnd, 0, static_cast<LONG>( size_t(this) ) );

        // publish the window handle under a well defined name
        nEnv *env;
        if ( 0 != (env = (nEnv *) nKernelServer::Instance()->New("nenv","/sys/env/hwnd"))) 
        {
            env->SetP( this->hWnd );
        }
    }
...
nWin32WindowHandler::RestoreWindow()
{
    ...
    if (iconName)
    {
        ...
        if (icon)
        {
            SetClassLong(this->hWnd, GCL_HICON, static_cast<LONG>( size_t(icon) ) );
        }
    }
    ...
    else if (this->displayMode.GetType() == nDisplayMode2::ChildWindow && this->parentHwnd)
    {
        ...
        this->displayMode.SetWidth( static_cast<ushort>( w ) );
        this->displayMode.SetHeight( static_cast<ushort>( h ) );
    }
    else
    {
...
nWin32WindowHandler::Trigger()
{
    ...
    {
        // get actual state of qualifier flags SHIFT, CONTROL and MENU
        this->refInputServer->ResetQualifierFlag();
        int keystate = 0;
        keystate = GetKeyState( VK_SHIFT );
        if( ( keystate & 0x80 ) == 0x80 )
        {
            this->refInputServer->SetQualifierFlag( N_KEY_SHIFT );
        }

        keystate = GetKeyState( VK_CONTROL );
        if( ( keystate & 0x80 ) == 0x80 )
        {
            this->refInputServer->SetQualifierFlag( N_KEY_CONTROL );
        }

        keystate = GetKeyState( VK_MENU );
        if( ( keystate & 0x80 ) == 0x80 )
        {
            this->refInputServer->SetQualifierFlag( N_KEY_MENU );
        }
    }
...
*/
bool
nWin32WindowHandler::OnSetFocus()
{
    return false;
}
...
*/
bool
nWin32WindowHandler::OnKillFocus()
{
    return false;
}
...
nWin32WindowHandler::WinProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam)
{
    ...
    nWin32WindowHandler* self = reinterpret_cast<nWin32WindowHandler*>( size_t(GetWindowLong(hWnd, 0)) );
        ...
        case WM_KEYDOWN:
            if (self && self->refInputServer.isvalid()) 
            {
                ...
                if (ie) 
                {
                    ...
                    self->refInputServer->SetQualifierFlag( key );
                }
            }
            break;
        ...
        case WM_KEYUP:
            if (self && self->refInputServer.isvalid()) 
            {
                ...
                if (ie) 
                {
                    ...
                    ie->SetKey(key);
                    self->refInputServer->LinkEvent(ie);
                    self->refInputServer->UnsetQualifierFlag( key );
                }
            }
            break;
