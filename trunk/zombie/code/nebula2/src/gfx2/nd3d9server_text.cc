...
nD3D9Server::OpenTextRenderer()
{
    ...
    n_dxverify2(
        D3DXCreateSprite(this->d3d9Device, &this->d3dSprite),
        "D3DCreateSprite() failed!" );
}
...
nD3D9Server::CloseTextRenderer()
{
    ...
    TextNode* textNode;
    while( 0 != (textNode = (TextNode*) this->textNodeList.RemHead()))
    {
        n_delete(textNode);
    }
...
nD3D9Server::DrawTextBuffer()
{
    ...
    {
        N_OUTPUTDEBUGSTRING("nD3D9Shader::DrawTextBuffer()\n");

        n_assert(this->d3d9Device);
        this->d3d9Device->SetRenderState(D3DRS_ZENABLE, FALSE);
...
nD3D9Server::GetTextExtent(const char* text)
{
    ...
    if (this->refFont.isvalid())
    {
        ...
        RECT rect = { 0 };
        /*int h = */h = d3dFont->DrawTextA(NULL, tmp, -1, &rect, DT_LEFT | DT_NOCLIP | DT_CALCRECT, 0);
