...
static int tclPipeCommand(Tcl_Interp *interp, const char *cmd, int objc, Tcl_Obj *CONST objv[])
{
    ...
    newObjv[0] = Tcl_NewStringObj(cmd, static_cast<int>( strlen(cmd) ) );
...
tclcmd_Sel(ClientData /*cdata*/, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
{
    ...
    else
    {
        ...
        if (obj)
        {
            ...
            nTclServer::Instance->SetCurrentTargetObject(obj);
        } 
        else 
        {
            tclUnknownObject(interp, Tcl_GetString(objv[1]));
...
tclcmd_Psel(ClientData /*cdata*/, Tcl_Interp *interp, int objc, Tcl_Obj *CONST /*objv*/[])
{
    ...
    {
        nObject *obj = nTclServer::kernelServer->GetCwd();
        if (obj && obj->IsA("nroot")) 
        {
            nRoot * root = static_cast<nRoot *>(obj);
            Tcl_SetResult(interp, (char*) root->GetFullName().Get(), TCL_VOLATILE);
            retval = TCL_OK;
        }
    }    
    return retval;
...
tclcmd_Dir(ClientData /*cdata*/, Tcl_Interp *interp, int objc, Tcl_Obj *CONST /*objv*/[])
{
    ...
    {
        nObject *cwd = nTclServer::kernelServer->GetCwd();
        n_assert(cwd);
        nRoot *obj;
        Tcl_Obj *res = Tcl_GetObjResult(interp);
        for (obj = (static_cast<nRoot *>(cwd))->GetHead(); obj; obj = obj->GetSucc()) 
        {
            const char *n = obj->GetName();
            Tcl_AppendToObj(res, (char *)n, static_cast<int>( strlen(n) ) );
    ...
    for (int i=0; i<num_args; i++)
    {
        switch (arg->GetType())
        {
            ...
            case nArg::String:
            {
                ...
                Tcl_Obj *so = Tcl_NewStringObj((char *)s, static_cast<int>( strlen(s) ) );
            ...
            case nArg::String:
                {
                    ...
                    Tcl_SetStringObj(res,(char *)s, static_cast<int>( strlen(s) ) );
                }
                break;
...
tclcmd_Unknown(ClientData cdata, Tcl_Interp *interp, int objc, Tcl_Obj *CONST objv[])
{
    ...
    {
        o = nTclServer::Instance->GetCurrentTargetObject(); // use the nObject if one is set
        if (!o)
        {
