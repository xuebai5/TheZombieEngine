...
nAnimState::Sample(float time, 
                   ...
                   int N_IFDEF_ASSERTS(keyArraySize))
{
    n_assert(animSource);
    n_assert(keyArray);
    ...
    /// @todo ma.garcias- disable if not using anim clips
    //<TODO>
    ...
    //</TODO>
        ...
        for (clipIndex = 0; clipIndex < numClips; clipIndex++)
        {
            ...
            if (clipWeight > 0.0f)
            {
                ...
                animSource->SampleCurves(time, this->animGroupIndex, firstCurveIndex, numCurves, scratchKeyArray, loopType, velocityFactor);
                ...
                for (curveIndex = 0; curveIndex < numCurves; curveIndex++)
                {
                    ...
                    if (animCurve.GetIpolType() == nAnimation::Curve::Quat)
                    {
                        ...
                        {
                            /// @todo - all those vector4/quaternion transfers are inefficient
...
*/
void
nAnimState::SetLeaderState(int leaderAnimStateIdx)
{
    this->leaderState = leaderAnimStateIdx;
}
...
*/
int
nAnimState::GetLeaderState() const
{
...
*/
const vector3 
nAnimState::GetTotalMotionMotion() const
{
    if (this->refMotion.isvalid())
    {
        int numKeys = this->refMotion->GetGroupAt(0).GetNumKeys();
        vector4 key = this->refMotion->GetKeyAt(numKeys-1);
        vector4 initial = this->refMotion->GetKeyAt(0);
        key -= initial;
...
*/
const vector3 
nAnimState::GetMotionAt(int key[2], float inbetween) const
{
    if (this->refMotion.isvalid())
    {
        vector4 initial = this->refMotion->GetKeyAt(key[0]);
        vector4 final = this->refMotion->GetKeyAt(key[1]);

        if (key[0] < key[1])
        {
            initial += (final - initial)*inbetween;
        }
        else
        {
            if( key[0] != key[1] )
            {
                initial += final*inbetween;
            }
        }

        return vector3(initial.x, initial.y, initial.z);
    }
...
*/
void 
nAnimState::SetFollowJoint(int jointIdx)
{
    this->followedJointIdx = jointIdx;
}
...
*/
int 
nAnimState::GetFollowJoint() const
{
    return this->followedJointIdx;
}
