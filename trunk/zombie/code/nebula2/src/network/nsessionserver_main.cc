#include "precompiled/pchnnetwork.h"
//------------------------------------------------------------------------------
...
nSessionServer::Trigger()
{
    ...
    if (this->ipcServer->Poll())
    {
        ...
        while (this->ipcServer->GetMsg(recvMsg, fromClientId))
        {
            ...
            while ( 0 != (curMsg = recvMsg.GetNextString()));
        }
    }
...
nSessionServer::BroadcastIdentity()
{
    ...
    if (timeSinceLastBroadcast > 1.0f)
    {
        ...
        n_assert( (strlen(msg) + 1)<= INT_MAX );
        nIpcBuffer ipcBroadcastMsg(msg, static_cast<int>( strlen(msg) + 1 ) );
