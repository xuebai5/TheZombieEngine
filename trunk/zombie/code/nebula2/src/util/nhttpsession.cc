...
nHttpSession::DoDialupStuff()
{
    ...
    else
    {
        ...
        if (env)
        {
            ...
            hwnd = static_cast<HWND>( env->GetP() );
        }
...
nHttpSession::Connect()
{
    ...
    if (0 == this->hInternet)
    {
        ...
        nString errorMsg( "InternetOpen() failed! : error " );
        errorMsg.AppendInt( err );
...
nHttpSession::AddRequestHeaders(HINTERNET hRequest, const nString& headers)
{
    ...
    if (!headers.IsEmpty())
    {
        BOOL result = HttpAddRequestHeaders(hRequest, headers.Get(), ULONG_MAX, HTTP_ADDREQ_FLAG_ADD);
...
nHttpSession::ReadResponse(HINTERNET hRequest, nString& response)
{
    ...
    while (readSuccess && InternetQueryDataAvailable(hRequest, &numBytesToRead, 0, 0) && (numBytesToRead > 0))
    {
        ...
        response.AppendRange(buffer, static_cast<ushort>( numBytesToRead ) );
        n_free(buffer);
    }
    return true;
}
