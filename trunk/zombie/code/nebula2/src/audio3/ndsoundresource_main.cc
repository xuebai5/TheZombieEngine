...
nDSoundResource::LoadResource()
{
    ...
    if ( this->freqControl )
    {
        creationFlags |= DSBCAPS_CTRLFREQUENCY;
    }
    if (!this->streaming)
    {
...
//        }
        ...
        if (FAILED(hr))
        {
            NLOG(resource, (NLOGUSER | 0, "nDSoundServer::LoadResource(): Creating streaming sound '%s' failed!", this->GetFilename().Get()));
            return false;
        }
