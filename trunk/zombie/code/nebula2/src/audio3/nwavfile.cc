...
mmioProc(LPSTR lpstr, UINT uMsg, LPARAM lParam1, LPARAM lParam2)
{
    ...
    switch (uMsg)
    {
        case MMIOM_OPEN:
            {
                if (0 == (lpMMIOInfo->dwFlags & (MMIO_WRITE | MMIO_READWRITE)))
                {
                    ...
                    if (file->Open(filename.Get(), "rb"))
                    {
                        ...
                        // old from OS: lpMMIOInfo->adwInfo[0] = (DWORD) file;
                        // @todo JJ: if in 64 bit processor, store also in the adwInfo[1]. Try to do more clean with some #if
                        memcpy( lpMMIOInfo->adwInfo,&file,sizeof(nFile*));
        ...
        case MMIOM_CLOSE:
            {
                // @todo JJ: if in 64 bit processor, store also in the adwInfo[1]. Try to do more clean with some #if
                // old from OS: nFile* file = (nFile*) lpMMIOInfo->adwInfo[0];
                nFile* file;
                memcpy( &file, lpMMIOInfo->adwInfo,sizeof(nFile*));
    ...
    m_dwSize(0)
{
    ...
    {
        N_IFDEF_ASSERTS( LPMMIOPROC p = ) mmioInstallIOProc(mmioFOURCC('N','E','B','2'), mmioProc, MMIO_INSTALLPROC);
        n_assert(p);
...
nWavFile::Open(const nString& filename)
{
    ...
    {
        NLOG(resource, (NLOGUSER | 0, "nWavFile::Open(): failed to open file '%s'!", filename.Get()));
        return false;
    }
    ...
    if (!this->ReadMMIO())
    {
        ...
        NLOG(resource, (NLOGUSER | 0, "nWavFile::Open(): not a wav file (%s)!", filename.Get()));
        return false;
    }
...
nWavFile::Reset(LONG pos)
{
    ...
    if (-1 == mmioSeek(this->m_hmmio, pos, SEEK_CUR))
    {
        return false;
    }
