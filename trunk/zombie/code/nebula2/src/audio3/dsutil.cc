...
//-----------------------------------------------------------------------------
// Name: DSEnum()
// Desc: Callback for sound device enumeration
//-----------------------------------------------------------------------------
BOOL CALLBACK DSEnum(LPGUID /*lpGuid*/,
                    LPCSTR /*lpcstrDescription*/,
                    LPCSTR /*lpcstrModule*/,
                    LPVOID lpContext)
{
    *((int*)lpContext) = *((int*)lpContext) + 1;
    return true;
}
                                   ...
                                   DWORD dwCoopLevel )
{
    ...
    // Enumerate devices to see if there is one
    int numEnumeratedDevices = 0;
    hr = DirectSoundEnumerate( DSEnum, &numEnumeratedDevices);
    if( FAILED( hr = DirectSoundCreate8( NULL, &m_pDS, NULL ) ) )
        return DXTRACE_ERR( TEXT("DirectSoundEnumerate"), hr );
    if ( numEnumeratedDevices == 0 )
    {
        return S_FALSE;
    }
                               ...
                               DWORD dwNumBuffers )
{
    ...
    if ( ! pWaveFile->Open(strWaveFileName) )
    {
        hr = E_INVALIDARG;
        goto LFail;
    }
...
// Desc: Checks to see if any buffer is playing and returns TRUE if it is.
//-----------------------------------------------------------------------------
DWORD CSound::GetStatus(int index)
{
    LPDIRECTSOUNDBUFFER buf = this->GetBuffer( index );
    if ( !buf )
    {
        return 0;
    }
    ...
    DWORD result;
    if ( FAILED( buf->GetStatus( &result ) ) )
    {
        return 0;
    }
    return result;
...
//-----------------------------------------------------------------------------
// Name: CSound::GetDuration()
// Desc: Return durations in seconds
//-----------------------------------------------------------------------------
float CSound::GetDuration()
{
    LPDIRECTSOUNDBUFFER buf = this->GetBuffer( 0 );

    WAVEFORMATEX format;
    DWORD alloc, written;
    alloc = sizeof( WAVEFORMATEX );
    n_dxverify( buf->GetFormat(NULL, alloc, &written ) );
    n_dxverify( buf->GetFormat(&format, alloc, &written ) );

    DSBCAPS bufCaps;
    bufCaps.dwSize = sizeof( DSBCAPS );
    buf->GetCaps( &bufCaps );

    float nsamples = ( bufCaps.dwBufferBytes * 8.0f ) / ((float) format.wBitsPerSample * format.nChannels);

    return  nsamples / format.nSamplesPerSec;
}

//-----------------------------------------------------------------------------
// Name: CSound::SetTrackPosition()
// Desc: Set buffer (index) track position in seconds
//-----------------------------------------------------------------------------
void 
CSound::SetTrackPosition( int index, float seconds )
{
    LPDIRECTSOUNDBUFFER buf = this->GetBuffer( index );
    ...
    float nsamples = format.nSamplesPerSec * seconds;
    float nbytes = nsamples * ((float) format.wBitsPerSample * format.nChannels / 8.0f);
    if ( nbytes > 0.0f && nbytes <= bufCaps.dwBufferBytes )
    {
        buf->SetCurrentPosition( DWORD( nbytes ) );
    }
}
...
HRESULT CStreamingSound::FillBufferWithSound( LPDIRECTSOUNDBUFFER pDSB, BOOL bRepeatWavIfBufferLarger )
{
    ...
    m_pWaveFile->Reset( this->resetPos );
    dwWavDataRead = m_pWaveFile->Read(pDSLockedBuffer, dwDSLockedBufferSize);
    m_pwfx = m_pWaveFile->GetFormat();
...
CStreamingSound::CheckStreamUpdate()
{
    ...
    DWORD playCursor;
    DWORD writeCursor;
    n_dxverify( m_apDSBuffer[0]->GetCurrentPosition(&playCursor, &writeCursor) );
...
// Desc: Resets the sound so it will begin playing at the beggining of file
//-----------------------------------------------------------------------------
...
//-----------------------------------------------------------------------------
// Name: CStreamingSound::GetDuration()
// Desc: Return durations in seconds
//-----------------------------------------------------------------------------
float CStreamingSound::GetDuration()
{
    LPDIRECTSOUNDBUFFER buf = this->GetBuffer( 0 );
    ...
    n_assert( this->m_pWaveFile );
    int fileSize = this->m_pWaveFile->GetSize();
    float nsamples = ( fileSize * 8.0f ) / ((float) format.wBitsPerSample * format.nChannels);

    return  nsamples / format.nSamplesPerSec;
}

//-----------------------------------------------------------------------------
// Name: CStreamingSound::SetTrackPosition()
// Desc: Set buffer (index) track position in seconds
//-----------------------------------------------------------------------------
void 
CStreamingSound::SetTrackPosition( int /*index*/, float seconds )
{
    LPDIRECTSOUNDBUFFER buf = this->GetBuffer( 0 );
    ...
    if ( nbytes > 0.0f && nbytes <= fileSize )
    {
        this->SetResetPositon( LONG( nbytes ) );
        this->Reset();

        m_apDSBuffer[0]->SetCurrentPosition( 0L );
    }
}

//-----------------------------------------------------------------------------
// Name: CStreamingSound::SetResetPosition()
// Desc: Set reset position for further Reset() calls
//-----------------------------------------------------------------------------
void
CStreamingSound::SetResetPositon( DWORD pos )
{
    this->resetPos = pos;
}
