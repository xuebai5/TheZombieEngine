#include "precompiled/pchnnpktool.h"
//------------------------------------------------------------------------------
/**
...
writeTocEntry(nFile* file, nNpkTocEntry* tocEntry)
{
    ...
    n_assert( strlen(entryName) <= INT_MAX );
    int entryNameLen = static_cast<int>( strlen(entryName) );
...
//        n_printf("=> writing dir entry '%s'\n", entryName);
        ...
        n_assert( entryNameLen <= SHRT_MAX );
        file->PutShort( static_cast<short>( entryNameLen ) );
...
printTocEntry(nNpkTocEntry* entry, int& recursionDepth)
{
    ...
    if (nNpkTocEntry::DIR == type)
    {
        ...
        } while ( 0 != (childEntry = entry->GetNextEntry(childEntry)));
...
BuildInternalPath(nNpkTocEntry* entry, char* buffer, int bufSize)
{
    ...
    for (; depth >= 0; --depth)
    {
        ...
        n_assert( strlen(curName) <= INT_MAX );
        int curNameLen = static_cast<int>( strlen(curName) );
