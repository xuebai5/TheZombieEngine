        ...
        do
        {
            ...
            if( this->CheckName( entryName ) )
            {
                ...
                else if (nDirectory::DIRECTORY == entryType)
                {
                    ...
                    {
                        // recurse
                        this->GenerateToc(subDir, entryName);
                        subDir->Close();
                        subDirOk = true;
                    }
                    n_delete(subDir);
...
nNpkBuilder::WriteTocEntry(nNpkTocEntry* tocEntry, bool noTopLevelName)
{
    ...
    if (nNpkTocEntry::DIR == entryType)
    {
        ...
        this->npkFile->PutShort((short)entryNameLen);
...
//------------------------------------------------------------------------------
/**
    @param mask new mask filter string
*/
void
nNpkBuilder::AddMaskFilter( const nString & mask )
{
    this->maskFilters.Append( mask );
}
...
*/
bool
nNpkBuilder::CheckName( const nString & name ) const
{
    for( int i = 0; i < this->maskFilters.Size() ; ++i )
    {
        if( 0 != strstr( name.Get(), this->maskFilters[ i ].Get() ) )
        {
            return false;
        }
    }
