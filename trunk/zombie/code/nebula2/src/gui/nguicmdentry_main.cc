...
nGuiCmdEntry::ExecuteCommand()
{
    ...
    // set new local cwd
    nRoot* newCwd = static_cast<nRoot*>( kernelServer->GetCwd() );
    if (newCwd != this->GetCwd())
    {
        this->SetCwd(static_cast<nRoot*>( kernelServer->GetCwd() ));
...
nGuiCmdEntry::UpdateTabComplete()
{
    ...
    nString wordUnderCursor = this->editLine.GetWordToCursor();
    ...
    if (path.IsEmpty())
    {
        dir = static_cast<nRoot*>( kernelServer->GetCwd() );
    }
    else
    {
        dir = static_cast<nRoot*>( kernelServer->Lookup(path.Get()) );
    }
    if (dir)
    {
        nRoot* child;
