...
nRemoteServer::Close()
{
    ...
    while ( 0 != (curContext = (nClientContext*) this->clientContexts.RemHead()) )
    {
        n_delete(curContext);
    }
...
nRemoteServer::Broadcast(const char* str)
{
    ...
    {
        n_assert(this->ipcServer);
        nIpcBuffer msg(str, static_cast<int>( strlen(str) + 1 ) );
...
*/
void
nRemoteServer::SetClientCwd(int clientId, nObject* cwd)
{
    nClientContext* context = this->GetClientContext(clientId);
    context->SetCwd(cwd);
}
        ...
        if (this->ipcServer->Poll())
        {
            ...
            while (this->ipcServer->GetMsg(recvMsg, fromClientId))
            {
                ...
                if (curString)
                {
                    ...
                    while ( 0 != (curString = recvMsg.GetNextString()) );
                }
                else
                {
