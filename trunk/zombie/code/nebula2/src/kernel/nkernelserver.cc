...
//------------------------------------------------------------------------------
NCREATELOGLEVEL(kernel, "Kernel", false, 1);
...
nKernelServer::NewUnnamedObject(const char* className)
{
    ...
    if( !cl )
    {
        if ( this->callBackClassNotFound )
        {
            this->callBackClassNotFound( className);
            cl = this->OpenClass(className);
        }
    }
    nObject *obj = 0;
    if (cl)
    {
        obj = (nObject *) cl->NewObject();
    }
    this->Unlock();
    return obj;
}
...
nKernelServer::Lookup(const char* path)
{
    ...
    if (path[0])
    {
        ...
        else
        {
            if (this->cwd)
            {
                if (this->cwd->IsA(this->root->GetClass()))
                {
                    cur = static_cast<nRoot*> (this->cwd);
                }
                else
                {
                    n_assert2_always("Mad and ugly programmer: trying to resolve relative paths with nObjects");
                    cur = 0;
                }
            }
            else
        ...
        while ( ( 0 != (nextPathComponent = strtok(str, "/")) ) && cur)
        {
            if (str)
            {
                str = NULL;
...
nKernelServer::CheckCreatePath(const char* className, const char *path, bool dieOnError)
{
    ...
    else if (this->cwd->IsA(this->root->GetClass()))
    {
        parent = static_cast<nRoot *> (this->cwd);
    }
    else
    {
        n_assert2_always("Mad and ugly programmer: trying to resolve relative paths with nObjects");
    ...
    if (curPathComponent)
    {
        ...
        while ( 0 != (nextPathComponent = strtok(NULL, "/")) )
        {
    ...
    callBackClassNotFound(0)
{
    ...
    /*nRoot *log          = */              this->New("nlogclassregistry", "/sys/servers/log");
...
nKernelServer::AddClass(const char *superClassName, nClass *cl)
{
    ...
    // Special case: Root classes have "nil" parent
    if( strcmp("nil",superClassName) ) {
...
/**
    ...
    is not loaded, call to callback for load one and if it is not posiblen then 0 is returned.
...
*/
nClass*
nKernelServer::IsClassLoaded(const char* className)
{
    n_assert(className);
    this->Lock();
    nClass *cl = (nClass *) this->classList.Find(className);
...
*/
nRoot*
nKernelServer::New(const char* className, const char* path, bool initInstance)
{
    ...
    if (o && initInstance)
    {
        o->InitInstance(nObject::NewInstance);
    }
    n_assert(o);
    this->Unlock();
    return o;
...
*/
nObject*
nKernelServer::New(const char* className, bool initInstance)
{
    n_assert(className);
    this->Lock();
...
*/
nRoot*
nKernelServer::NewNoFail(const char* className, const char *path, bool initInstance)
{
    n_assert(className && path);
    this->Lock();
...
*/
nObject*
nKernelServer::NewNoFail(const char* className, bool initInstance)
{
    n_assert(className);
    this->Lock();
    nObject *obj = this->NewUnnamedObject(className);
...
/*
    ...
    @param  file    persistent object file in host filesystem
    @return         pointer to created object, or 0
     - 23-Feb-05   Cris    const char* -> nFile*
     - 08-Oct-98   floh    created
     - 04-Oct-98   floh    char * -> const char *
     - 11-Nov-98   floh    implementiert
*/
nObject*
nKernelServer::Load(nFile* file, bool initInstance)
{
    n_assert(file);
    this->Lock();
    nObject* obj = this->persistServer->LoadObject(file, 0);

    // we must call InitInstance for the root object in the file
    // since this does not get a call to EndObject()
    if (obj && initInstance)
    {
        obj->InitInstance(nObject::LoadedInstance);
    }
...
*/
nObject*
nKernelServer::Load(const char* path, bool initInstance)
{
    n_assert(path);
    nFile* file = this->GetFileServer()->NewFileObject();
    if (!file->Open( path, "r"))
    {
        file->Release();
        return 0;
    ...
    nObject* obj = this->Load( file, initInstance );
    file->Close();
    file->Release();
    return obj;
}
...
*/
nRoot*
nKernelServer::LoadAs(nFile* file, const char* name, bool initInstance)
{
    n_assert(file && name);
    this->Lock();
    nObject* obj = this->persistServer->LoadObject(file, name);
    n_assert2(obj && obj->IsA("nroot"), file->GetFilename().Get() );
...
*/
nRoot*
nKernelServer::LoadAs(const char* path, const char* name, bool initInstance)
{
    ...
    nRoot* obj = this->LoadAs( file, name,initInstance );
    file->Close();
    file->Release();
    return obj;
}
...
*/
void
nKernelServer::SetCwd(nObject * o)
{
    NLOG(kernel, (1, "SetCwd(0x%x) class=%s name=%s", o, 
        o ? o->GetClass()->GetName() : "", 
        o && o->IsA("nroot") ? (static_cast<nRoot*>(o))->GetFullName() : "" ))
    this->Lock();
    if (o)
    {
        this->cwd = o;
    ...
    nPersistServer * ps = this->GetPersistServer();
    if (ps)
    {
        nScriptServer * ss = ps->GetScriptServer();
        if (ss)
        {
            ss->SetCurrentTargetObject(this->cwd);
        }
    }
    this->Unlock();
}
...
{
    NLOG(kernel, (1, "GetCwd=0x%x class=%s name=%s", this->cwd, 
        this->cwd ? this->cwd->GetClass()->GetName() : "", 
        this->cwd && this->cwd->IsA("nroot") ? (static_cast<nRoot*>(this->cwd))->GetFullName() : "" ))
    return this->cwd;
}
...
*/
void
nKernelServer::PushCwd(nObject* o)
{
    NLOG(kernel, (1, "PushCwd(0x%x) class=%s name=%s", o, 
        o ? o->GetClass()->GetName() : "", 
        o && o->IsA("nroot") ? (static_cast<nRoot*>(o))->GetFullName() : "" ))
    this->Lock();
...
{
    NLOG(kernel, (1, "PopCwd() prevCwd=0x%x class=%s name=%s", this->cwd, 
        this->cwd ? this->cwd->GetClass()->GetName() : "", 
        this->cwd && this->cwd->IsA("nroot") ? (static_cast<nRoot*>(this->cwd))->GetFullName() : "" ))
    this->Lock();
    nObject *prevCwd = this->cwd;
...
nKernelServer::AddModule(const char *name,
                         nClass * (*_init_func)(const char *, nKernelServer *))
{
    this->Lock();
    ...
    if (!cl)
    {
        ...
        nClass *cl2 = (nClass *) this->classList.Find(name);
        n_assert(cl2 && cl == cl2);
    }
    this->Unlock();
}
...
*/
void
nKernelServer::SetCallBackClassNotFound( CallBack callBackClassNotFound)
{
    n_assert( callBackClassNotFound );
    this->callBackClassNotFound = callBackClassNotFound;
}

//------------------------------------------------------------------------------
/**
*/
nKernelServer::CallBack
nKernelServer::GetCallBackClassNotFound()
{
    return this->callBackClassNotFound;   
}