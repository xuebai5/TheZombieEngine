...
nRoot::GetFullName() const
{
    ...
    while ( (0 != (cur = cur->GetParent())) && (i < maxDepth));
...
nRoot::SaveFile(nFile* file)
{
    ...
    bool retval = false;
    if (ps->BeginObject(this, file, true)) 
    {
        // ...the usual behaviour...
        nRoot *c;
    ...
    {
        n_error("nRoot::SaveFile(): BeginObject() failed!");
    }
...
*/
nObject*
nRoot::Clone(const char *name, bool initInstance)
{
    n_assert(name);
    nObject *clone = NULL;
    nPersistServer *ps = nKernelServer::Instance()->GetPersistServer();
    ...
    n_assert_return2(clone && this != clone, 0, ("kernel server: clone operation not succesful, name=%s class=%s", name, this->GetClass()->GetName() ));
    n_assert_return2(clone->GetClass() == clone->GetClass(), 0, ("Clone and cloned entity have different class"));

    /// it would be better to convert bool to enum ( CREATED, LOADED, CLONED )
    /// by the moment keep it as loaded
    if (clone && initInstance)
    {
        clone->InitInstance(nObject::LoadedInstance);
    }
...
*/
void
nRoot::BindDirtyDependence(nObject* receiver)
{
    this->BindSignal( nObject::SignalObjectDirty , receiver, &nObject::TriggerObjectDirty , 10);
    nRoot* curChild;
    for (curChild = this->GetHead(); curChild; curChild = curChild->GetSucc())
    {
        curChild->BindDirtyDependence(this);
    }
}
...
*/
void
nRoot::RecruseSetObjectDirty(bool dirty)
{
    this->SetObjectDirty(dirty);
    nRoot* curChild;
    for (curChild = this->GetHead(); curChild; curChild = curChild->GetSucc())
    {
        curChild->RecruseSetObjectDirty(dirty);
    }
}
