...
nWin32LogHandler::Open()
{
    ...
    if (nLogHandler::Open())
    {
        ...
        n_verify(S_OK == nWin32Wrapper::Instance()->SHGetFolderPath(0, // hwndOwner
...
*/
int
nWin32LogHandler::Assert(const char* msg, va_list argList)
{
    if (this->logFile && this->logFile->IsOpen())
    {
        char buf[1024];
    ...
    return MessageBox(0,msgBuf, this->appName.Get(),
...
nWin32LogHandler::PutMessageBox(MsgType type, const char* msg, va_list argList)
{
    ...
    if (MsgTypeError == type)
    {
        ...
        if (envHwnd)
        {
            HWND hwnd = static_cast<HWND>( envHwnd->GetP() );
            if (hwnd)
            {
...
nString
nWin32LogHandler::GetAbsolutePath()
{
    return this->path;
}
