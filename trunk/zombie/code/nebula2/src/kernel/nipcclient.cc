...
nIpcClient::ApplyBlocking(bool b)
{
    if (b)
    {
        // enable blocking
            ...
            N_IFDEF_ASSERTS(int res =) 
            ioctlsocket(this->sock, FIONBIO, &falseAsUlong);
    ...
    else
    {
        // disable blocking
            ...
            N_IFDEF_ASSERTS(int res =)
            ioctlsocket(this->sock, FIONBIO, &trueAsUlong);
        #elif defined(__LINUX__) || defined(__MACOSX__)
            int flags;
            flags = fcntl(this->sock, F_GETFL);
            flags |= O_NONBLOCK;
...
nIpcClient::Connect(nIpcAddress& addr)
{
    if (!this->isConnected)
    {
        ...
        res = send(this->sock, msg, static_cast<int>( strlen(msg) + 1 ) , 0);
        if (res == -1)
        {
            // initial send failed!
...
nIpcClient::Disconnect()
{
    if (this->isConnected)
    {
        ...
        int res = send (this->sock, cmd, static_cast<int>( strlen(cmd) + 1 ), 0);
        if (res == -1)
        {
