...
*/
bool 
nSignalRegistry::AddSignals(nSignalRegistry * sr)
{
    n_assert(sr);
    nHashList* signalList = sr->GetSignalList();
    if (signalList)
    {
        nSignal * signal = static_cast<nSignal *> (signalList->GetHead());
        while(signal)
        {
            nSignal * clonedSignal = static_cast<nSignal *> (signal->Clone());
            this->AddSignal(clonedSignal);
            signal = static_cast<nSignal *> (signal->GetSucc());
        }
    }
