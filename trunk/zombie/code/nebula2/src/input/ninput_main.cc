...
nInputServer::ExportDefaultKeyboard(void)
{
    ...
    int i = 0;
    while ( 0 != (s = key_remaptable[i].str))
    {
        char buf[128];
...
nInputServer::~nInputServer()
{
    ...
    // kill input mappings
    nInputMapping *im;
    while ( 0 != (im = (nInputMapping *) (this->im_list.RemHead())))
    {
        n_delete(im);
    }
    ...
    // kill input states
    nInputState *is;
    while ( 0 != (is = (nInputState *) (this->is_list.RemHead())))
    {
        n_delete(is);
    }
...
*/
void
nInputServer::SetQualifierFlag( nKey key )
{
    switch( key )
    {
    case N_KEY_CONTROL:
        this->qualifierFlags |= nInputMapping::N_IMSTATE_CTRL_FLAG;
        break;
    case N_KEY_SHIFT:
        this->qualifierFlags |= nInputMapping::N_IMSTATE_SHIFT_FLAG;
        break;
    case N_KEY_MENU:
        this->qualifierFlags |= nInputMapping::N_IMSTATE_MENU_FLAG;
        break;
    }
}
...
*/
void
nInputServer::UnsetQualifierFlag( nKey key )
{
    switch( key )
    {
    case N_KEY_CONTROL:
...
*/
void
nInputServer::ResetQualifierFlag()
{
    this->qualifierFlags = 0;
}
