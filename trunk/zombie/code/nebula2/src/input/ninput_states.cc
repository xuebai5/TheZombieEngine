...
*/
const char*
nInputServer::BuildInputMappingName(const char* ieStr, const char* isStr, char* buf, int N_IFDEF_ASSERTS(bufSize))
{
...
bool nInputServer::Map(const char *ie_str, const char *is_str)
{
    ...
    if (!im) 
    {
        ...
        if (qu_part)
        {
            if (strcmp(qu_part, "ctrl")       == 0) mod_flags |= nInputMapping::N_IMSTATE_CTRL_FLAG;
            else if (strcmp(qu_part, "shift") == 0) mod_flags |= nInputMapping::N_IMSTATE_SHIFT_FLAG;
            else if (strcmp(qu_part, "menu")   == 0) mod_flags |= nInputMapping::N_IMSTATE_MENU_FLAG;
            else
            {
                n_printf("nInputServer::Map(%s,%s): invalid modifier '%s'!\n",
                        ie_str,is_str,qu_part);
            }
        }

        ie_part  = strchr(buf,':');
        // special case for only flag keys
        if (ie_part)
        {
            ++ie_part;
            if (strcmp(ie_part, "ctrl")       == 0) mod_flags |= nInputMapping::N_IMSTATE_CTRL_FLAG;
            else if (strcmp(ie_part, "shift") == 0) mod_flags |= nInputMapping::N_IMSTATE_SHIFT_FLAG;
            else if (strcmp(ie_part, "menu")   == 0) mod_flags |= nInputMapping::N_IMSTATE_MENU_FLAG;
        }
    ...
    char *scr_kw = "script:";
    int len_scr_kw = static_cast<int>( strlen(scr_kw) );
...
nInputServer::DoInputMapping(void)
{
         ...
         im = (nInputMapping *) im->GetSucc())
    {
        ...
        } while ( 0 != (cur_ie = this->NextIdenticalEvent(ie,cur_ie)));
    }
