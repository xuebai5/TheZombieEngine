    ...
    if (!this->emitterPool.Empty())
    {
        ...
        while (emitterIter != this->emitterPool.End())
        {
            ...
            if (!emitter->IsAlive() || emitter->GetFatalException())
            {
                ...
                //if (!emitter->IsAlive() && emitter->GetKeepAlive() > 0.0f)
                if (!emitter->IsAlive())// && emitter->GetEmissionDuration() > 0.0f)
                {
                    nTime elapsedTime = curTime - emitter->GetLastTimeAlive();
                    keepAlive = elapsedTime < emitter->GetEmissionDuration();
                //  keepAlive = elapsedTime < emitter->GetKeepAlive();
                }
            ...
            {
                emitter->SetLastTimeAlive(curTime);
