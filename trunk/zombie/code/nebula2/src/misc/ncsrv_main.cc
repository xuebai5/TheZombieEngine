...
{
    this->cursorPos++;
    int len = static_cast<int>( strlen(this->inputBuffer) );
...
nConServer::EditEnd()
{
    this->cursorPos = static_cast<int>( strlen(this->inputBuffer) );
}
...
nConServer::EditLine(nInputEvent* inputEvent)
{
    ...
    if ((inputEvent->GetType() == N_INPUT_KEY_CHAR) &&
        (inputEvent->GetChar() >= ' '))
    {
        this->EditInsertChar( static_cast<char>( inputEvent->GetChar() ) );
        killEvent = true;
    }
...
nConServer::Trigger()
{
    if (this->consoleOpen)
    {
        ...
        if (inputEvent)
        {
            ...
            while ( 0!= (inputEvent = nextInputEvent));
        }
    }
}
