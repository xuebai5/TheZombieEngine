...
//------------------------------------------------------------------------------
//  nspatialtestviewerapp.cc
//  (C) 2003 RadonLabs GmbH
//------------------------------------------------------------------------------
#include "nspatial/nspatialtestviewerapp.h"
...
/**
*/
nSpatialTestViewerApp::nSpatialTestViewerApp(nKernelServer* ks) :
    ...
    screenshotID(0)
{
    ...
    //camera.SetNearPlane(4.0f);
}

//------------------------------------------------------------------------------
/**
*/
nSpatialTestViewerApp::~nSpatialTestViewerApp()
{
    if (this->isOpen)
    {
        this->Close();
...
nSpatialTestViewerApp::Open()
{
    ...
    this->renderContext.SetRootNode((nSceneNode*) this->refRootNode.get()->GetHead()); // stdlight
    this->refRootNode->RenderContextCreated(&this->renderContext);

    // initialize spatial server
    this->refSpatialServer.get()->Initialize(kernelServer, this->refVarServer.get(), this->refSceneServer.get());
    // catch the necessary spatial information from the scene
    nTransformNode* light = (nTransformNode*) this->refRootNode.get()->GetHead();
    this->refSpatialServer.get()->CatchSpatialInfo((nSceneNode*) light->GetSucc());
...
nSpatialTestViewerApp::Run()
{
    ...
    while (this->refGfxServer->Trigger() && running)
    {
        ...
        if (!this->refGfxServer->InDialogBoxMode())
        {
            ...
            viewContext.SetTransform(viewMatrix);
            if (this->refSceneGraph->BeginAttach())
            {
                //this->refSceneServer->Attach(&this->renderContext); // attach the light
                this->refSceneGraph->Attach(&this->renderContext); // attach the light
                // attach visible objects
                this->refSpatialServer.get()->VisibilityTrigger(this->camera, viewMatrix);
                //this->refSceneServer->EndScene();
                this->refSceneGraph->EndAttach();
...
*/
void
nSpatialTestViewerApp::TransferGlobalVariables()
{
...
*/
void
nSpatialTestViewerApp::HandleInput(float frameTime)
{
    ...
    if (inputServer->GetButton("togglevisibility"))
    {
        this->refSpatialServer.get()->ToggleVisibility();
    }
}
...
*/
void
nSpatialTestViewerApp::HandleInputMaya(float frameTime)
{
    nInputServer* inputServer = this->refInputServer.get();
...
*/
void
nSpatialTestViewerApp::HandleInputFly(float frameTime)
{
    nInputServer* inputServer = this->refInputServer.get();
    if (frameTime <= 0.0001f)
    {
...
*/  
void
nSpatialTestViewerApp::InitOverlayGui()
{
