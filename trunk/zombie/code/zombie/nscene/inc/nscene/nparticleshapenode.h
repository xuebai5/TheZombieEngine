...
public:
    ...
    /// called by nSceneServer when object is rendered
    virtual bool Render(nSceneGraph* sceneGraph, nEntityObject* entityObject);
    /// render geometry
    bool RenderGeometry(nSceneGraph* sceneGraph, nEntityObject* entityObject);
    ...
    bool RenderTransform(nSceneGraph* sceneGraph, nEntityObject* entityObject);
	...
	/// get the emitter
    nParticleEmitter* GetEmitter(nEntityObject* entityObject);
    #ifndef NGAME
    /// return the texture path for edit in property grid
    nString GetParticleTexture();
    /// for edit the texture path in property grid
    void SetParticleTexture(const char* /*path*/);
    /// return the mesh path for edit in property grid
    nString GetParticleMesh();
    /// for edit the  mesh path in property grid. Each vertex of mesh is a spawn point
    void SetParticleMesh(const char* /*path*/);

    /// return source blend function, property grid
    int GetFuncSrcBlend();
    /// set the source blend function, property grid
    void SetFuncSrcBlend( int );

     /// return destination blend function, property grid 
    int GetFuncDestBlend();
    /// set the destination blend function, property grid
...
{
    this->globalScale = f;
//    this->SetScale(vector3(f, f, f)); // FIXME that came from ntransformnode
}
