...
PyObject*
GetReturnableObject(NebulaObject* pyobj, nObject* /*nebobj*/)
{
...
NebulaObject*
CreatedObjectsList_GetObject(nObject* real_neb_obj)
{
    ...
    PyObject *ptr_as_int = (PyObject *)PyInt_FromLong(static_cast<long>( size_t(real_neb_obj) ));
    ...
    {
        n_assert2_always("error getting wrapper nebula object from weakref\n");
    }
...
int
CreatedObjectsList_AddObject(NebulaObject *nebula_object, nObject* real_neb_obj)
{
    n_assert(real_neb_obj);
    ...
    Py_DECREF(weak_ref); // no need anymore

    return res;
}

int
CreatedObjectsList_DeleteObject(nObject* /*real_neb_obj*/)
{
    // XXX: !!!
    return 0;
/*    if (NULL == real_neb_obj)
...
pythoncmd_New(PyObject * /*self*/, PyObject *args)
{
    ...
    // Extract one or two strings from the Python args object
    // Also, ":<desc>" for a descriptive error if an exception is raised.
    if (PyArg_ParseTuple(args, "s|s:New", &className, &objectName ))
    {
        // if there is a second parameter
        if( objectName )
        {
            // create the nObject in the N.O.H
            o = nPythonServer::kernelServer->NewNoFail(className, objectName);
        }
        else
        {
            // create the nObject
            o = nPythonServer::kernelServer->New(className);
        }
...
pythoncmd_Delete(PyObject * /*self*/, PyObject *args)
{
    ...
    if (PyArg_UnpackTuple(args, "del", 1, 1, &object))
    {
        if (PyString_Check(object))
        {
            char * objectName = PyString_AS_STRING(object);
        ...
        else if (PyObject_TypeCheck(object, &Nebula_Type))
        {
            nObject * o = (nObject *) NebulaObject_GetPointer((NebulaObject*)object);
            if (o)
            {
                n_assert(o->IsA("nobject"));
                o->Release();
                Py_INCREF(Py_None);
                result = Py_None;
            }
            ...
            {
                PyErr_SetString(PyExc_Exception, "Couldn't delete the object");
            }
        }
        else
        {
...
pythoncmd_Sel_(PyObject *args, bool set_cwd) 
{
    ...
    {
        nObject * obj = nPythonServer::kernelServer->GetCwd();
        if (obj && !obj->IsA("nroot"))
        {
            PyErr_SetString(PyExc_Exception, "SEL: Unable to create a NebulaObject");
            return NULL;
        }

        res_neb = NebulaObject_New(obj);
        if (res_neb == NULL)
        {
            PyErr_SetString(PyExc_Exception, "SEL: Unable to create a NebulaObject");
        ...
        {
            return GetReturnableObject(res_neb, obj);
        }
    }
    ...
    else
    {
        ...
        {
            object = (nRoot *) NebulaObject_GetPointer((NebulaObject*)tempo);
            ...
            if (object)
            {
                ...
                if (set_cwd)
                {
                    ...
                    o = static_cast<nRoot *> (nPythonServer::kernelServer->GetCwd());
                    n_assert(o == object);
                }
                else
                {
...
pythoncmd_Psel(PyObject * /*self*/, PyObject *args)
{
    ...
    if (PyArg_ParseTuple(args, ":psel"))
    {
        ...
        nRoot * root = static_cast<nRoot*>(obj);
        result = PyString_FromString(root->GetFullName().Get());
    }
    return result;
}
...
pythoncmd_GetCwdObject(PyObject * /*self*/, PyObject *args)
{
    ...
    if (PyArg_ParseTuple(args, ":GetCwd"))
    {
        ...
        if (obj)
        {
            NebulaObject *res_neb = NebulaObject_New(obj);
            if (res_neb == NULL)
            {
                PyErr_SetString(PyExc_Exception, "SEL: Unable to create a NebulaObject");
                return NULL;
            }
            result = GetReturnableObject(res_neb, obj);
        }
        else
        {
            // Report failure
...
pythoncmd_Dir(PyObject * /*self*/, PyObject *args)
{
    ...
    {
        nObject *cwd = nPythonServer::kernelServer->GetCwd();
        if (cwd && cwd->IsA("nroot"))
        {
            // Display contents of path acquired from GetCwd()
            result = PyList_New(0);
            for (nRoot * o = (static_cast<nRoot*>(cwd))->GetHead(); o; o = o->GetSucc())
            {
...
pythoncmd_Set(PyObject * /*self*/, PyObject *args)
{
    ...
    if (commandPyString && PyString_Check(commandPyString))
    {
        ...
        if (*commandName != '\0')
        {
            ...
            {
                o = nPythonServer::Instance->GetCurrentTargetObject();
                if (NULL == o)
                {
...
//-----------------------------------------------------------------------------
/**
    Implementation of 'PyBindSignal' command for nCmdProtoPython.
*/
PyObject*
pythoncmd_PyBindSignal( PyObject * /*self*/, PyObject * args )
{
    PyObject * emitter, * object;
    char * signalName, * method;
    int priority;

    if( ! PyArg_ParseTuple( args, "OsOsi:pyBindSignal", &emitter, &signalName, &object, &method, &priority ) )
    {
        n_message( "Usage: pyBindSignal( emitter, signalname, callable object, priority )" );
        Py_RETURN_NONE;
    }

    if( ! NebulaObject_Check( emitter ) )
    {
        n_message( "Emitter is not a Nebula Object" );
        Py_RETURN_NONE;
    }

    nObject * obj = reinterpret_cast<NebulaObject*>(emitter)->nebula_object->get();
    n_assert( obj );
    if( !obj )
    {
        Py_RETURN_NONE;
    ...
    nSignalRegistry * regs = obj->GetSignalRegistry();
    n_assert( regs );
    if( !regs )
    {
        Py_RETURN_NONE;
    ...
    nSignal * signal = regs->FindSignalByName( signalName );
    n_assert( signal );
    if( !signal )
    {
        Py_RETURN_NONE;
    ...
    nSignalBindingPython * binding = n_new( nSignalBindingPython( object, method, priority, signal->GetProtoDef() ) ); 
    n_assert( binding );
    if( binding )
    {
        if( ! obj->BindSignal( signal, binding ) )
        {
            n_delete( binding );
        }
    }
...
//-----------------------------------------------------------------------------
/**
    Implementation of 'PyUnbindTargetObject' command for nCmdProtoPython.
*/
PyObject*
pythoncmd_PyUnbindTargetObject( PyObject * /*self*/, PyObject * args )
{
    PyObject * target, * emitter;
    char * signalName;

    //if( ! PyArg_ParseTuple( args, ":pyUnbindTargetObject" ) )
    if( ! PyArg_ParseTuple( args, "OsO:pyUnbindTargetObject", &emitter, &signalName, &target ) )
    {
        n_message( "Usage: pyUnbindTargetObject( emitter, signalname, callable object )" );
        Py_RETURN_NONE;
    }
    ...
    nSignalBinding * binding = 0;
    binding = nSignalBindingPython::FindBinding( target );

    if( binding )
    {
        obj->UnbindSignal( signal->GetId(), binding );
    }
