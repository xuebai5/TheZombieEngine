...
//-------------------------------------------------------------------
/**
    - 26-Aug-04   Miquel Angel Rujula   created
*/
//-------------------------------------------------------------------

void nOctree::SetSize(vector3 minCorner, vector3 maxCorner)
{
    this->tree_root->minCorner = minCorner;
    this->tree_root->maxCorner = maxCorner;
}
...
void nOctree::collapse(nOctNode *n)
{
    ...
    if (n->c[0]) {   
        ...
        for (i=0; i<8; i++) {
            ...
            nOctElement *oe;
            while ( 0 != (oe = (nOctElement *) n->c[i]->elm_list.GetHead())) {
