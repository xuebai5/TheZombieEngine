...
class BldScanner(Scanner):
    ...
    targetStates = {'annotate'              : 't_annotate',
                    ...
                    'setwarningslevel'      : 't_setwarningslevel',
                    'setlinkeroptionsvc'    : 't_setlinkeroptionsvc',
                    'setcompileroptionsvc'  : 't_setcompileroptionsvc',
                    'setpch'                : 't_setpch',
                    'setpchfile'            : 't_setpchfile', 
                    'setstacksize'          : 't_setstacksize'}
    ...
    workspaceStates = {'annotate'                   : 'w_annotate', 
                       ...
                       'setglobalcompileroptionsvc' : 'w_setglobalcompileroptionsvc' }
    ...
    def SetGlobalCompilerOptionsVC (self, text):
        self.CheckTargetNameDefined()
        self.workspaces[len(self.workspaces) - 1].compilerOptions.append(text)
        if not self.InBlock():
            self.begin('workspace') # switch back to parent state
    ...
    def SetWarningsLevel(self, text):
        self.CheckTargetNameDefined()
        self.targets[len(self.targets) - 1].warningsLevel = text
        self.begin('target') # switch back to parent state
    
    def SetLinkOptionsVC (self, text):
        self.CheckTargetNameDefined()
        self.targets[len(self.targets) - 1].linkerOptions.append(text)
        if not self.InBlock():
            self.begin('target') # switch back to parent state

    def SetCompilerOptionsVC (self, text):
        self.CheckTargetNameDefined()
        self.targets[len(self.workspaces) - 1].compilerOptions.append(text)
        if not self.InBlock():
            self.begin('target') # switch back to parent state
    ...
    def SetPch (self, text):
        self.CheckTargetNameDefined()
        if text == 'true':
            target = self.targets[len(self.workspaces) - 1]
            moduleName = 'pch' + target.name
            module = Module(moduleName, target.name)
            module.files.append(moduleName + '.cc')
            module.headers.append(moduleName + '.h')
            module.putInPkg = False
            module.dir = 'precompiled'
            self.modules.append(module)
            target.modules.append(moduleName)
            target.pchlevel = "3"
            target.pchfile = 'precompiled/' + moduleName + '.h'
        else:
            pass
        if not self.InBlock():
            self.begin('target') # switch back to parent state

    def SetPchFile (self, text):
        # set 'text' as precompiled header for the target
        self.CheckTargetNameDefined()
        self.targets[len(self.targets) - 1].pchlevel = "3"
        self.targets[len(self.targets) - 1].pchfile = str(text)
        if not self.InBlock():
            self.begin('target') # switch back to parent state

    def SetStackSize (self, number):
        self.CheckTargetNameDefined()
        self.targets[len(self.targets) - 1].stacksize = str(number)
        if not self.InBlock():
            self.begin('target') # switch back to parent state
    ...
    name = alphanum + Rep(alphanum | Str('_') | Str('.'))
    filename = Rep(alphanum | Any('_.-')) # Allows subdirs
    spaceortab = Any(' \t')
    ...
    vcoptions = Alt(Rep1(AnyBut('" \t\n')), 
                     Str('"') + Rep(AnyBut('"') | space + Str('"')))
    ...
    lexicon = Lexicon([ (Str('beginworkspace'), BeginWorkspace),
                        ...
                        State('t_setwarningslevel', [
                            (digit, SetWarningsLevel),
                            (AnyChar, IGNORE)
                        ]),
                        State('t_setlinkeroptionsvc', [
                            (Str('{'), StartBlock),
                            (Str('}'), EndBlock),
                            (annotation, SetLinkOptionsVC),
                            (AnyChar, IGNORE)
                        ]),
                        State('t_setcompileroptionsvc', [
                            (Str('{'), StartBlock),
                            (Str('}'), EndBlock),
                            (annotation, SetCompilerOptionsVC),
                            (AnyChar, IGNORE)
                        ]),
                        State('t_setpch', [
                            (Str('{'), StartBlock),
                        ...
                        State('w_setglobalcompileroptionsvc', [
                            (Str('{'), StartBlock),
                            (Str('}'), EndBlock),
                            (annotation, SetGlobalCompilerOptionsVC),
                            (AnyChar, IGNORE)
                        ]),
                        State('t_setstacksize', [
                            (Str('{'), StartBlock),
                            (Str('}'), EndBlock),
                            (integer, SetStackSize),
                            (AnyChar, IGNORE)
                        ]),                       
                        (space, IGNORE),
                        (comment, IGNORE),
