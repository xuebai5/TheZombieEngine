...
class BuildSys:
    ...
    def __init__(self, homeDir, fileName, enableGUI = False):
        ...
        # global options for all workspaces
        self.globalCplOptions = [] 
    ...
    def Run(self, generatorName, workspacesToBuild, extraArgs={}):
        try:
                ...
                for workspaceName in workspacesToBuild:
                    if workspaceName not in availableWorkspaces:
                        self.logger.error('Workspace %s is undefined!', 
                                          workspaceName)
                        return
            # get global option from targets
            for target in self.targets.values():
                self.globalCplOptions.extend(target.compilerOptions)

            # create the generator and get it running
            generator = self.generatorFactory.GetGenerator(generatorName)
            if extraArgs.has_key('target') and extraArgs.has_key('module'):
                generator.Generate(workspacesToBuild, extraArgs['target'], extraArgs['module'])
            else:
    ...
    def findCodeDirs(self):
        ...
        searchPath = os.path.join(self.homeDir, 'code', 'zombie', '*') + os.sep
        #print 'Search Path: ' + searchPath
        self.zombieDirs = glob.glob(searchPath)

        searchPath = os.path.join(self.homeDir, 'code', 'exporter', '*') + os.sep
        #print 'Search Path: ' + searchPath
        self.exporterDirs = glob.glob(searchPath)

        searchPath = os.path.join(self.homeDir, 'code', 'conjurer', '*') + os.sep
        #print 'Search Path: ' + searchPath
        self.conjurerDirs = glob.glob(searchPath)

        searchPath = os.path.join(self.homeDir, 'code', 'renaissance', '*') + os.sep
        self.renaissanceDirs = glob.glob(searchPath)
        ...
        while True:
            ...
            for i in range(len(self.projectDirs)):
                ...
                if string.upper(tail) == 'CVS' or string.lower(tail) == '.svn':
                    matchIdx = i
                    break;
            if matchIdx != -1:
        ...
        # same with the zombie paths
        while True:
            matchIdx = -1
            for i in range(len(self.zombieDirs)):
                self.zombieDirs[i] = string.rstrip(self.zombieDirs[i], os.sep)
                head, tail = os.path.split(self.zombieDirs[i])
                if string.upper(tail) == 'CVS' or string.lower(tail) == '.svn':
                    matchIdx = i
                    break;
            if matchIdx != -1:
                del self.zombieDirs[matchIdx]
            else:
                break;

        # same with the exporter paths
        while True:
            matchIdx = -1
            for i in range(len(self.exporterDirs)):
                self.exporterDirs[i] = string.rstrip(self.exporterDirs[i], os.sep)
                head, tail = os.path.split(self.exporterDirs[i])
                if string.upper(tail) == 'CVS' or string.lower(tail) == '.svn':
                    matchIdx = i
                    break;
            if matchIdx != -1:
                del self.exporterDirs[matchIdx]
            else:
                break;

        # same with the conjurer paths
        while True:
            matchIdx = -1
            for i in range(len(self.conjurerDirs)):
                self.conjurerDirs[i] = string.rstrip(self.conjurerDirs[i], os.sep)
                head, tail = os.path.split(self.conjurerDirs[i])
                if string.upper(tail) == 'CVS' or string.lower(tail) == '.svn':
                    matchIdx = i
                    break;
            if matchIdx != -1:
                del self.conjurerDirs[matchIdx]
            else:
                break;

        # same with the renaissance paths
        while True:
            matchIdx = -1
            for i in range(len(self.renaissanceDirs)):
                self.renaissanceDirs[i] = string.rstrip(self.renaissanceDirs[i], os.sep)
                head, tail = os.path.split(self.renaissanceDirs[i])
                if string.upper(tail) == 'CVS' or string.lower(tail) == '.svn':
                    matchIdx = i
                    break;
            if matchIdx != -1:
                del self.renaissanceDirs[matchIdx]
            else:
                break;
            
        #print self.projectDirs
        #print self.zombieDirs
