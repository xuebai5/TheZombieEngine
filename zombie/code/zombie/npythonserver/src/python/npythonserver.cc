...
extern PyObject* pythoncmd_PyBindSignal(PyObject *self, PyObject *args);
extern PyObject* pythoncmd_PyUnbindTargetObject(PyObject *self, PyObject *args);
...
static PyMethodDef NebulaMethods[] = {
    ...
    {"setTrigger",    pythoncmd_SetTrigger, METH_VARARGS, NULL},   // Trigger callback
    {"pyBindSignal",  pythoncmd_PyBindSignal, METH_VARARGS, NULL},
    {"pyUnbindTargetObject", pythoncmd_PyUnbindTargetObject, METH_VARARGS, NULL},

    {"__CreatedObjectsList_weakref_callback__",  CreatedObjectsList_weakref_callback, METH_VARARGS, NULL}, 
...
nPythonInitialize(const char* moduleName)
{
    ...
    if (!nPythonServer::Instance) 
    {
        ...
        nPythonServer *ps = (nPythonServer *) nPythonServer::kernelServer->Lookup("/sys/servers/npythonscript");
        if (!ps) 
        {
            ps = (nPythonServer *) nPythonServer::kernelServer->New("npythonserver", "/sys/servers/npythonscript");
            n_assert(ps);
...
*/
bool
nPythonServer::BeginWrite(nFile * file, nObject* obj, nCmd * cmd)
{
    n_assert(file);
    n_assert(obj);
    ...
    if (cmd)
    {
        ...
        snprintf(buf, sizeof(buf), "# $parser:npythonserver$ $class:%s$\n", cmdStr.Get());
    }
    else if (obj)
    {
...
*/
bool
nPythonServer::WriteBeginNewObject(nFile *file, nObject *o, nObject * owner)
{
...
*/
bool
nPythonServer::WriteBeginNewObjectCmd(nFile *file, nObject *o, nObject *owner, nCmd *cmd)
{
    // write new statement with custom constructor
...
*/
bool
nPythonServer::WriteEndObject(nFile *file, nObject *o, nObject *owner)
{
    _indent(--this->indent_level, this->indent_buf);

    bool success = nScriptServer::WriteEndObject(file, o, owner);
    ...
    for (int i = 0; i < numArgs; i++)
    {
        ...
        switch (arg->GetType())
        {
            ...
            case nArg::String:
            {
                ...
                if (strPtr != NULL)
                {
                    n_assert( strlen(strPtr) <= USHRT_MAX );
                    strLen = static_cast<ushort>( strlen(strPtr) );
                }
            ...
            case nArg::Object:
            {
                ...
                {
                    char buf[N_MAXPATH];
                    nRoot * root = static_cast<nRoot *> (o);
                    sprintf(buf, "'%s'", root->GetFullName().Get());
                }
                else
                {
                    sprintf(buf, " null");
