...
void lua_print( const char *msg, ... ) {
    ...
    nLuaServer::Instance->AppendOutput( nString( msg ) );
...
int _luaDispatch(lua_State* L, nObject* obj, nCmdProto* cmd_proto, bool print)
{
    ...
    {
        NLOG(persistence, (NLOGUSER | 0, "Wrong number of arguments for command: %s", 
    ...
    {
        NLOG(persistence, (NLOGUSER | 0, "Incorrect arguments for: %s",
                    cmd_proto->GetProtoDef()));
        lua_settop(L, 0);
        lua_pushnil(L);
        cmd_proto->RelCmd(cmd);
    ...
    if (!obj->Dispatch(cmd))
    {
        NLOG(persistence, (NLOGUSER | 0, "Could not dispatch the command: %s",
                    cmd_proto->GetProtoDef()));
        lua_pushnil(L);
        cmd_proto->RelCmd(cmd);
        return 1;
...
int luacmd_NewThunk(lua_State* L)
{
    if (2 == lua_gettop(L))
    {
        ...
        {
            n_message("Usage is newthunk('class', 'name') or newthunk('class')");
            lua_settop(L, 0);
            lua_pushnil(L);
            return 1;
        }
    ...
    else if (1 == lua_gettop(L))
    {
        if (!lua_isstring(L, -1))
        {
            n_message("Usage is new('class', 'name') or new('class')");
            lua_settop(L, 0);
            lua_pushnil(L);
            return 1;
        }
        ...
        lua_settop(L, 0);
        nObject* obj = nLuaServer::kernelServer->New(className);
        if (!obj)
        {
            n_message("Could not create object of class '%s'\n", 
                        className);
            lua_pushnil(L);
        }
        else
...
int luacmd_PinThunk(lua_State* L)
{
    ...
    else
    {
        ...
        nLuaServer::AddThunk(L, (void*)obj);
        lua_settop(L, 0);
        lua_pushboolean(L, true);
    }
    return 1;
...
int luacmd_Sel(lua_State* L)
{
    ...
    if (!o)
    {
        lua_print("Could not select object\n");
        lua_pushnil(L);
    }
    else if (!nLuaServer::IsAnRoot( o ))
    {
        lua_print("Could not select object, not nroot\n");
        lua_pushnil(L);
    }
    else
    {        
    ...
    nLuaServer::Instance->SetCurrentTargetObject(o);
...
int luacmd_Psel(lua_State* L)
{
    // takes no arguments
    // returns 1 thunk
    nObject* o = nLuaServer::kernelServer->GetCwd();
    if (o)
    {
...
int luacmd_Get(lua_State* L)
{
    ...
    bool isRoot = nLuaServer::IsAnRoot( o );
    n_assert(o && isRoot);
    if (!o)
    {
...
int luacmd_Dir(lua_State* L)
{
    ...
    lua_settop(L, 0);
    nObject* cwd = nLuaServer::kernelServer->GetCwd();
    if (!cwd || (cwd && !nLuaServer::IsAnRoot( cwd )))
    {
    ...
    int i = 0;
    nRoot* o = 0;
    for (o=static_cast<nRoot *>(cwd)->GetHead(); o; o=o->GetSucc())
    {
        lua_pushnumber(L, i++);
...
int luacmd_CmdDispatch(lua_State* L)
{
    ...
    return _luaDispatch(L, obj, cmdproto, false);
}
...
int luacmd_Call(lua_State* L)
{
    ...
    nObject* obj = nLuaServer::Instance->GetCurrentTargetObject();
    if (!obj)
...
int luacmd_ConCall(lua_State* L)
{
    ...
    nObject * obj = nLuaServer::kernelServer->GetCwd();
    const char* cmdname = lua_tostring(L, 1);
    nClass* cl = obj->GetClass();
    nCmdProto* cmd_proto = (nCmdProto*) cl->FindCmdByName(cmdname);
    if (!cmd_proto)
    ...
    return _luaDispatch(L, obj, cmd_proto, true);
}
...
int luacmd_Lookup(lua_State* L)
{
    ...
    if (!root)
    {
        // the error is show outside, because it could be ok not found nothing
...
{
    if ( ((1 != lua_gettop(L)) || !lua_isstring(L, -1)) &&
         ((2 != lua_gettop(L)) || !lua_isstring(L, -1) || !lua_isboolean(L, -2)) )
    {
        n_message("Usage is mangle('path') or mangle('path',boolRead)");
        lua_settop(L, 0);
        lua_pushnil(L);
        return 1;
    }
    ...
    if (2 == lua_gettop(L))
    {
        read = lua_toboolean(L, -2);
    }
    nString path = nFileServer2::Instance()->ManglePath(lua_tostring(L, -1), read);
    if (path.IsEmpty())
    {
...
int luacmd_IsZombieThunk(lua_State* L)
{
    ...
    if ((1 != lua_gettop(L)) || !lua_istable(L, -1))
    {
      lua_print("Usage is IsZombieThunk(thunk)\n");
      lua_settop(L, 0);
      lua_pushnil(L);
      return 1;
    }
...
int luacmd_PushCwd(lua_State* L)
{
    ...
    {
        lua_print("Usage is pushcwd('path') or pushcwd(thunk)\n");
        lua_settop(L, 0);
        lua_pushboolean(L, false);
        return 1;
    }
    ...
    else // string path case
    {
        ...
        {
            lua_print("pushdir: %s not found!", lua_tostring(L, -1));
            lua_settop(L, 0);
            lua_pushboolean(L, false);
        }
    }
...
//--------------------------------------------------------------------
//  luacmd_NotFoundIndex()
//--------------------------------------------------------------------
int luacmd_NotFoundIndex(lua_State * L)
{
    bool valid = true;

    // check lua parameters
    if( 2 != lua_gettop(L) || !( lua_istable(L, -2) && lua_isstring(L, -1) ) )
    {
        lua_print("Usage is _notfoundindex( table, index )\n");
        valid = false;
    }
    ...
    {
        // get the method name
        methodName.Set(lua_tostring( L, -1 ) );

        // check if table is a thunk
        lua_pushstring(L, "_");
        lua_rawget(L, -3);

        valid = lua_isuserdata(L, -1);
        n_assert2(valid, "nRef userdata not found in thunk!");
    }
    ...
    if( valid )
    {
        ...
        // get nebula object from thunk
        nRef<nObject> * ref = (nRef<nObject>*)lua_unboxpointer(L, -1);

        if (ref && ref->isvalid())
        {
            bool found = false;
            if( nScriptClassServer::Instance() )
            {
                // get script function from script class server
                found = nScriptClassServer::Instance()->GetScript( ref->get(), methodName, functionName );

                if( found )
                {
                    // get the function from lua
                    nArray<nString> table;
            ...
            if( ! found )
            {
                nCmdProto * cmdProto = ref->get()->GetClass()->FindCmdByName( methodName.Get() );
                if( cmdProto )
                {
...
//  luacmd_RelGrab()
//--------------------------------------------------------------------
int luacmd_RelGrab(lua_State * L)
{
    bool valid = true;
    ...
    if( 2 != lua_gettop(L) || !( lua_istable(L, -2) && lua_isstring(L, -1) ) )
    {
        lua_print("Usage is _relgrab( table, index )\n");
        valid = false;
    }
    ...
    if( valid )
    {
        ...
        if (ref && ref->isvalid())
        {
            if( nLuaServer::IsAnRoot( ref->get() ) )
            {
                nRoot * root = static_cast<nRoot*>( ref->get() );
...
//  luacmd_NopFunction()
//--------------------------------------------------------------------
int luacmd_NopFunction(lua_State * /*L*/)
{
    return 0;
}
...
int luaobject_Emit( lua_State *L )
{
    ...
    if( !obj )
    {
        lua_print( "Object not found in object:emit( ... )\n");
        lua_settop(L, 0);
        return 0;
    }
    ...
    {
        lua_print( "Usage is object:emit( 'signal', ... )\n");
        lua_settop(L, 0);
        return 0;
    }
    ...
    if( !cmd_proto )
    {
        lua_print( "Signal not found in object:emit( '%s', ... )\n", signal.Get() );
        lua_settop(L, 0);
        return 0;
    }
...
int luaobject_Post( lua_State *L )
{
    ...
    {
        lua_print( "Usage is object:post( time, 'signal', ... )\n");
        lua_settop(L, 0);
        return 0;
    }
    ...
    if (!nLuaServer::StackToInArgs(L, cmd))
    {
        lua_print("Incorrect arguments for: %s\n", cmd_proto->GetProtoDef());
        lua_settop(L, 0);
        return 0;
    }
    ...
    if( signalServer )
    {
        ...
        {
            lua_print( "Post error, in object of class '%s', with signal '%s'", 
