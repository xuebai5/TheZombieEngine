...
NCREATELOGLEVEL ( luaRunLog, "Lua Run", true, 1 ) 
...
*/
bool nLuaServer::RunScript(nFile * file, nString& result)
{
    n_assert(file);
    ...
    if (!retval)
    {
        if (!result.IsEmpty())
        {
            n_message("nLuaServer::RunScript failed:\nfile: %s\nmessage:\n%s\n", 
                file->GetFilename().Get(), result.Get());
        }
        else
        {
