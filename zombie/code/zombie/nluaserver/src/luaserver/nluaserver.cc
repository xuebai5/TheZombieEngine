...
nClass *nLuaServer::nroot_cl = NULL;
...
extern int luacmd_NotFoundIndex(lua_State*);
extern int luacmd_RelGrab(lua_State*);
extern int luacmd_NopFunction(lua_State*);
...
nLuaServer::nLuaServer() :
    ...
    selgrab(false)
{
    nroot_cl = nKernelServer::Instance()->FindClass("nroot");
    this->indent_level = 0;
    this->indent_buf[0] = 0;
    ...
    reg_globalfunc(luacmd_RelGrab,          "relgrab");
    reg_globalfunc(luacmd_DeleteNRef,       "_delnref");
    reg_globalfunc(luacmd_NotFoundIndex,    "_notfoundindex");
    reg_globalfunc(luacmd_NopFunction,       "_nopfunction");
...
//------------------------------------------------------------------------------
bool
nLuaServer::BeginWrite(nFile * file, nObject* obj, nCmd * cmd)
{
    n_assert(file);
    n_assert(obj);
...
bool nLuaServer::WriteComment(nFile *file, const char *str)
{
    ...
    nString tmp = str;
    const char* line;
    for ( line = tmp.GetFirstToken("\n") ; line ; line = tmp.GetNextToken("\n") )
    {
...
//--------------------------------------------------------------------
bool nLuaServer::WriteBeginNewObject(nFile *file, nObject *o, nObject *owner)
{
    bool success = nScriptServer::WriteBeginNewObject(file, o, owner);
...
//  defined by command.
//--------------------------------------------------------------------
bool nLuaServer::WriteBeginNewObjectCmd(nFile *file, nObject *o, nObject *owner, nCmd *cmd)
{
    bool success = nScriptServer::WriteBeginNewObjectCmd(file, o, owner, cmd);
...
//--------------------------------------------------------------------
bool nLuaServer::WriteBeginSelObject(nFile *file, nObject *o, nObject *owner)
{
    n_assert(file);
    n_assert(o);
...
//  WriteEndObject()
//--------------------------------------------------------------------
bool nLuaServer::WriteEndObject(nFile *file, nObject *o, nObject *owner)
{
    _indent(--this->indent_level, this->indent_buf);

    return nScriptServer::WriteEndObject(file, o, owner);
}
            ...
            case nArg::Float:
                {
                    ...
                    {
                        n_assert2_always( "INF or NAN float value write in LUA" );
                        sprintf(buf, "%d", 0);
                    }
#else
                    sprintf(buf, "%.6f", value);
            ...
            case nArg::Object:
                {
                    ...
                    if (o) {
                        sprintf(buf, "lookup('/')%s", o->GetFullName().Get());
                        char*p(strchr(buf,')'));
                        while( 0 != (p = strchr( p, '/' )) )
                            *p = '.';
                    } else {
...
void nLuaServer::AddClassToCache(lua_State* L, nClass* clazz)
{
    ...
    // get cmds in all the hierarchy of clazz
    nArray<nHashList*> commands;
    do
    {
        commands.Append( clazz->GetCmdList() );
    }while( 0 != (clazz = clazz->GetSuperClass()) );

    // put cmds in upside-down order to allow children redefine 
    // parent cmds
    for( int i = commands.Size() - 1 ; i >= 0 ; --i )
    {
        protoList = commands[ i ];
        if (protoList)
        {
...
*/
bool nLuaServer::ThunkNebObject(lua_State* L, nObject* obj)
{
    n_assert(obj);
    ...
    nRef<nObject>* ref_Object = n_new(nRef<nObject>);
    ref_Object->set(obj);
    lua_boxpointer(L, ref_Object); // 3
    ...
    /* @todo: Remove the Class Cache. DON'T USE IT
    ...
    if (!lua_istable(L, -1))
    {
        ...
        nLuaServer::AddClassToCache(L, obj->GetClass());
...
*/
nObject* nLuaServer::UnpackThunkRoot(lua_State* L, int tableidx)
{
    n_assert(tableidx > 0);
...
bool nLuaServer::StackToArg(lua_State* L, nArg* arg, int index)
{
    ...
    switch (arg->GetType())
    {
        ...
        case nArg::Object:
        {
            ...
            {
                nObject* object = nLuaServer::UnpackThunkRoot(L, index);
                if (!object) 
                    return false;
                arg->SetO(object);
            }
...
//--------------------------------------------------------------------
void nLuaServer::SetCurrentTargetObject(nObject *obj)
{
    nScriptServer::SetCurrentTargetObject(obj);
    if (obj)
    {
        nLuaServer::FindThunk(L, (void*)obj);
        if (lua_isnil(L, -1)) // no thunk found? create it
        {
            lua_remove(L, -1);
...
//--------------------------------------------------------------------
void nLuaServer::SetNThis(void)
{
    // thunk is already on the stack
    lua_pushstring(L, "nthis");
    lua_insert(L, -2);
    lua_settable(L, LUA_GLOBALSINDEX);
}

//--------------------------------------------------------------------
void nLuaServer::UnsetNThis(void)
{
    lua_pushstring(L, "nthis");
    lua_pushnil(L);
    lua_settable(L, LUA_GLOBALSINDEX);
...
//--------------------------------------------------------------------
/**
  @brief Test if the object is an nRoot.
  @param obj The object to be tested.
  @return True if arg is an instance of nRoot or a derived class.

  This function exists only for performance reasons.
*/
bool nLuaServer::IsAnRoot( nObject *obj )
{
    return obj->IsA(nroot_cl);
}
