...
*/
void
n_initcmds_nSkinGeometryAnimator(nClass* cl)
{
    cl->BeginCmds();
    cl->AddCmd("v_setstateanim_is",         'SSAN', n_setstateanim);
    cl->AddCmd("s_getstateanim_i",          'GSAN', n_getstateanim);
...
{
    nSkinGeometryAnimator* self = (nSkinGeometryAnimator*) slf;
    const int i0 = cmd->In()->GetI();
    const char *s0 = cmd->In()->GetS();
    self->SetStateAnim(i0, s0);
}
...
*/
bool
nSkinGeometryAnimator::SaveCmds(nPersistServer* ps)
{
    if (nAnimator::SaveCmds(ps))
    {
        ...
        while (strNode)
        {
            ...
            cmd->In()->SetI(static_cast<unsigned int>(reinterpret_cast<size_t>(strNode->GetPtr())));
        ...
        if (numStates > 0)
        {
            ...
            for (stateIndex = 0; stateIndex < numStates; stateIndex++)
            {
                ...
                //--- setstateanim ---
                cmd = ps->GetCmd(this, 'SSAN');
                cmd->In()->SetI(stateIndex);
                cmd->In()->SetS(state.GetAnimFile());
                ps->PutCmd(cmd);
