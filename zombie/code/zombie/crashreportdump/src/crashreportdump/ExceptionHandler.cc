...
#include <dbghelp.h>
#include "crashreportdump/CrashFileNames.h"
								...
								LPCTSTR lpszMessage)
{
    ...
    if (GetUserDebugOption() == DebugUserOption) 
    {
        return EXCEPTION_CONTINUE_SEARCH;
    }

    if (GetUserDebugOption() == UnknowUserOption) 
    {
        if (MessageBox(NULL,"Application crashed:\nPress NO for DEBUG\nPress YES for SEND REPORT",NULL,MB_YESNO) == IDNO) 
        {
            return EXCEPTION_CONTINUE_SEARCH;
        }
    }

    // Write stack trace in log file
    n_printf("\n------- Nebula Stack ------------\n");
    n_puts(stack);
    n_printf("----------- END STACK --------------\n");
    if ( nKernelServer::ks && 
	...
	//pszFilePart = XCRASHREPORT_ERROR_LOG_FILE;
    lstrcpy( pszFilePart , XCRASHREPORT_ERROR_LOG_FILE ); // append to en of szModuleName
    ...
    // Add the Build Configuration to the errorlog file
    #ifdef __BUILD_CONF__

    #if __BUILD_CONF__ == 1
    #define __BUILD_CONF_STRING__ "Debug"
    #elif __BUILD_CONF__ == 2
    #define __BUILD_CONF_STRING__ "Debug Optimized"
    #elif __BUILD_CONF__ == 3
    #define __BUILD_CONF_STRING__ "Release Asserts"
    #elif __BUILD_CONF__ ==4
    #define __BUILD_CONF_STRING__ "Release"
    #endif

    #elif __BUILD_CONF__ "???"
    #endif

    hprintf(hLogFile, _T("Build Configuration: %s\r\n"),__BUILD_CONF_STRING__);
    hprintf(hLogFile, _T("svn revision: %s\r\n"),  N_RELEASE_SVN_REVISION );
    hprintf(hLogFile, _T("svn head url: %s\r\n"),  N_RELEASE_SVN_HEADURL );
    hprintf(hLogFile, _T("Build id: %s\r\n"),  N_RELEASE_BUILD_ID );
    hprintf(hLogFile, _T("Build date: %s %s\r\n"), N_RELEASE_BUILD_DATE, N_RELEASE_BUILD_TIME );
    hprintf(hLogFile, _T("Revision: %s\r\n"), N_RELEASE_SVN_REVISION_STR );
	...
	const TCHAR *pszCrashModuleFileName = _T("Unknown");
	...
	//pszFilePart = XCRASHREPORT_MINI_DUMP_FILE;
    lstrcpy( pszFilePart, XCRASHREPORT_MINI_DUMP_FILE); // apend to szModuleName
