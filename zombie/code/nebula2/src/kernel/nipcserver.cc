...
nIpcServer::~nIpcServer()
{
    ...
    while ( 0 != (ipcMiniServer = (nIpcMiniServer*) this->miniServerList.RemHead()) )
    {
    ...
    nMsgNode* msgNode;
    this->msgList.Lock();
    while ( 0 != (msgNode = (nMsgNode*) this->msgList.RemHead()) )
    {
        n_delete(msgNode);
    }
    this->msgList.Unlock();
...
nIpcServer::GetMsg(nIpcBuffer& msg, int& fromClientId)
{
    ...
    if (msgNode)
    {
        ...
        fromClientId = static_cast<int>( size_t( msgNode->GetPtr() ) );
