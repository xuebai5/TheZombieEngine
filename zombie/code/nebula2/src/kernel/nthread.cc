...
nThread::nThreadId nThread::mainThreadId(0);
...
#   endif
    ...
    if (!(this->isExtMsgList)) 
    {
        nMsgNode *nd;
        this->msgList->Lock();
        while ( 0 != (nd = (nMsgNode *) this->msgList->RemHead()) ) 
        {
            n_delete(nd);
        }
        this->msgList->Unlock();
...
nThread::MarkAsMainThread()
{
    ...
    n_assert2( mainThreadId != 0 , "I need the main thread isn't zero ");
}
...
*/
bool 
nThread::IsTheMainThread(nThreadId id)
{
    // mainThreadId is not initialize
    return (mainThreadId == 0) || id == mainThreadId;
}
