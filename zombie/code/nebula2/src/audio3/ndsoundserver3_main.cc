...
nDSoundServer3::UpdateListener(const nListener3& l)
{
    ...
    hr = this->dsListener->SetAllParameters(&(this->dsListenerProps), /*DS3D_DEFERRED*/DS3D_IMMEDIATE);
    n_assert(SUCCEEDED(hr));

    // Apply all deferred changes in sound sources made during this frame
    hr = this->dsListener->CommitDeferredSettings();
    n_assert(SUCCEEDED(hr));
...
nDSoundServer3::EndScene()
{
    ...
    {
        // @todo JJ this loop is waste of time. maintain a list of streamed playing sounds and update them directly
        this->lastStreamUpdateCheck = curTime;
        nResourcePool* rsrcPool = nResourceServer::Instance()->GetResourcePool(nResource::SoundResource);
        nRoot* cur;
    ...
    for(cat = 0; cat < NumCategorys; cat++)
    {
        if (this->masterVolumeDirty[cat])
        {
            ...
            nResourcePool* rsrcPool = nResourceServer::Instance()->GetResourcePool(nResource::SoundInstance);
            nRoot* cur;
            for (cur = rsrcPool->GetHead(); cur; cur = cur->GetSucc())
            {
