...
static void n_getbrushcolor(void* slf, nCmd* cmd);
static void n_setbrushcolor(void* slf, nCmd* cmd);
...
n_initcmds_nGuiSkin(nClass* cl)
{
    ...
    cl->AddCmd("ffff_getbrushcolor_s",          'LGBC', n_getbrushcolor);
    cl->AddCmd("v_setbrushcolor_sffff",         'LSBC', n_setbrushcolor);
    cl->EndCmds();
}
...
*/
static void
n_getbrushcolor(void* slf, nCmd* cmd)
{
    nGuiSkin* self = (nGuiSkin*) slf;
    nGuiResource * resource = self->FindBrush( cmd->In()->GetS() );
    if( resource )
    {
        const vector4& c = resource->GetColor();
        cmd->Out()->SetF(c.x);
        cmd->Out()->SetF(c.y);
        cmd->Out()->SetF(c.z);
        cmd->Out()->SetF(c.w);
...
*/
static void
n_setbrushcolor(void* slf, nCmd* cmd)
{
    nGuiSkin* self = (nGuiSkin*) slf;
    nGuiResource * resource = self->FindBrush( cmd->In()->GetS() );
    if( resource )
