...
nGuiMessageBox::OnShow()
{
    ...
    {
        this->SetDefaultBrush("window"); // FIXME: bgmessagebox on RL SDK?
    }
    ...
    //textLabel->SetFont(skin->GetWindowFont()); // SDK??
    ...
    if (this->type != NoButtons)
    {
        if ((OkCancel == this->type) || (Ok == this->type))
        {
            ...
            //okButton->SetFont(skin->GetButtonFont()); // SDK??
        ...
        if (OkCancel == this->type)
        {
            ...
            //cancelButton->SetFont(skin->GetButtonFont()); // SDK??
