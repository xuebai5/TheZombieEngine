...
static void n_newwindownamed(void* slf, nCmd* cmd);
...
*/
void
n_initcmds_nGuiServer(nClass* cl)
{
    cl->BeginCmds();
    ...
    cl->AddCmd("o_newwindownamed_sbs",             'NWWN', n_newwindownamed);
...
/**
    ...
    o(Window)
    @info
    Create a new window of the given class with a name
*/
static void
n_newwindownamed(void* slf, nCmd* cmd)
{
    nGuiServer* self = (nGuiServer*) slf;
    const char* s0 = cmd->In()->GetS();
    bool b0 = cmd->In()->GetB();
    const char* s1 = cmd->In()->GetS();
    cmd->Out()->SetO(self->NewWindowNamed(s0, b0, s1));
}
