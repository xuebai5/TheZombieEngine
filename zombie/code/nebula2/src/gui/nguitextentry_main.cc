...
nGuiTextEntry::~nGuiTextEntry()
{
    if (this->lineEditor)
    {
        ...
        this->lineEditor = 0;
        nGuiServer::Instance()->SetActiveTextEntry(false);
    }
}
...
nGuiTextEntry::SetActive(bool b)
{
    if (b)
    {
        if (!this->active)
        {
            ...
            nGuiServer::Instance()->SetActiveTextEntry(true);
        }
    }
    else
    {
...
nGuiTextEntry::Render()
{
    ...
    if (this->IsShown())
    {
        ...
        nGuiBrush * brushtopaint = &this->defaultBrush;
        ...
        {
            brushtopaint = &this->highlightBrush;
        }

        if (! this->IsEnabled()) brushtopaint = &this->disabledBrush;

        nGuiServer::Instance()->DrawBrush(screenSpaceRect, *brushtopaint);
