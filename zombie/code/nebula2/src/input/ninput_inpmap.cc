    ...
    : nHashNode(_name)
{
    ...
    // split mask in normal flags and qualifier flags
    this->maskFlags = this->mask & N_IMSTATE_QUALIFIERS;
    this->mask &= ~N_IMSTATE_QUALIFIERS;
}
...
//------------------------------------------------------------------------------
/**
    @returns true if input mapping is active
*/
bool 
nInputMapping::IsActive(void)
{
    // ONLY VALID AFTER PostTrigger()!
    n_assert(this->state_valid);
    bool valid = (this->state & this->mask) != 0;
    return valid && ( this->maskFlags == this->iserv->GetQualifierFlags() );
    //return (this->state & this->mask) == this->mask;
}
