...
static void n_newevent(void*, nCmd*);
static void n_neweventmotion(void*, nCmd*);
static void n_neweventbutton(void*, nCmd*);
...
/**
    ...
    device provides.
*/
NSCRIPT_INITCMDS_BEGIN(nInputServer)
    ...
    cl->AddCmd("v_newevent_iii",            'NEVT', n_newevent);
    cl->AddCmd("v_neweventmotion_iiiiff",   'NEVM', n_neweventmotion);
    cl->AddCmd("v_neweventbutton_iiiiffi",  'NEVB', n_neweventbutton);

    NSCRIPT_ADDCMD('SLTF', void, StartLogToFile, 1, (nString *), 0, ());
    NSCRIPT_ADDCMD('SLFF', void, StartLogFromFile, 1, (nString *), 0, ());
    NSCRIPT_ADDCMD('SLFL', void, StopLogFile , 0, (), 0, ());
    NSCRIPT_ADDCMD('ILFR', bool, IsLogFileRunning , 0, (), 0, ());
...
*/
static void n_newevent(void* slf, nCmd* cmd)
{
    nInputServer* self(static_cast<nInputServer*>(slf));
    
    nInputEvent *ie(self->NewEvent());

    n_assert2( ie, "Failed to create a new input server event" );


    ie->SetType( nInputType(cmd->In()->GetI()) );
    ie->SetDeviceId( cmd->In()->GetI() );
    ie->SetKey( nKey(cmd->In()->GetI()) );
...
/**
    ...
    v
    @info
    Generates a new event for motion.
*/
static void n_neweventmotion(void* slf, nCmd* cmd)
{
    nInputServer* self(static_cast<nInputServer*>(slf));
    ...
    float relx(cmd->In()->GetF());
    float rely(cmd->In()->GetF());

    ie->SetRelPos( float(x)/relx,float(y)/rely );
...
/**
    ...
    v
    @info
    Generates a new event for a button.
*/
static void n_neweventbutton(void* slf, nCmd* cmd)
{
    nInputServer* self(static_cast<nInputServer*>(slf));
