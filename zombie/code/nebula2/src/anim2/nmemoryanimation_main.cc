...
nMemoryAnimation::LoadNanim2(const char* filename)
{
    ...
    {
        NLOG(resource, (0, "nMemoryAnimation::LoadNanim2(): Could not open file %s\n", filename));
        file->Release();
        return false;
    }
    ...
    while (file->GetS(line, sizeof(line)))
    {
        ...
        else if (0 == strcmp(keyWord, "type"))
        {
            ...
            {
                NLOG(resource, (0, "nMemoryAnimation::LoadNanim2(): File %s has invalid type %s, must be 'nanim2'\n", filename, typeString));
                file->Close();
                file->Release();
                return false;
            }
        ...
        {
            NLOG(resource, (0, "nMemoryAnimation::LoadNanim2(): Unknown keyword %s in nanim2 file %s\n", keyWord, filename));
            file->Close();
            file->Release();
            return false;
        }
...
nMemoryAnimation::LoadNax2(const char* filename)
{
    ...
    {
        NLOG(resource, (0, "nMemoryAnimation::LoadNax2(): Could not open file %s!", filename));
        file->Release();
        return false;
    }
    ...
    if (magic != 'NAX2')
    {
        NLOG(resource, (0, "nMemoryAnimation::LoadNax2(): File %s is not a NAX2 file!", filename));
        file->Close();
        file->Release();
        return false;
    }
...
*/
void
nMemoryAnimation::SampleCurves(float time, int groupIndex, int firstCurveIndex, int numCurves, vector4* dstKeyArray, nAnimation::Group::LoopType loopType, float velocityFactor)
{
    // convert the time into 2 global key indices and an inbetween value
    const Group& group = this->GetGroupAt(groupIndex);
    int keyIndex[2];
    float inbetween;
    group.TimeToIndex(time, keyIndex[0], keyIndex[1], inbetween, loopType, velocityFactor);
...
*/
const vector4 
nMemoryAnimation::GetKeyAt(int keyIndex) const
{
