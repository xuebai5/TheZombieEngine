...
NCREATELOGLEVEL(VideoServer, "Direct Show", true, 1)
...
nDShowServer::Close()
{
    ...
    while (!this->videoPlayers.Empty())
    {
        this->DeleteVideoPlayer(*this->videoPlayers.Begin());
    }
...
nDShowServer::PlayFile(const char* filename)
{
    ...
    HWND hwnd(0);
    if( this->refHwnd->GetType() == nArg::Pointer )
    {
        hwnd = static_cast<HWND>( this->refHwnd->GetP() );
    }
    else
    {
        hwnd = reinterpret_cast<HWND>( size_t(this->refHwnd->GetI()) );
    }
    OAHWND ownerHwnd = (OAHWND) hwnd;
...
nDShowServer::Stop()
{
    ...
    HWND hwnd = static_cast<HWND>( this->refHwnd->GetP() );
...
nDShowServer::Trigger()
{
    ...
    //trigger video players through the ogg-theora / vmr9 interfaces
    nTime time = nTimeServer::Instance()->GetFrameTime();
    ...
    for (i = 0; i<videoPlayers.Size(); ++i)
    {
        ...
        if (currentPlayer->IsOpen())
        {
            NLOG(video, (0, "nDShowServer::Trigger decoding player: '%s', deltaTime: %.2f", currentPlayer->GetName(), deltaTime))
    ...
    #if 0
    //play a fullscreen video through the standard dshow interfaces
    if (this->IsPlaying())
    {
...
*/
nVideoPlayer*
nDShowServer::NewVideoPlayer(const char *name)
{
    ///@todo ma.garcias - check extension to use VMR9Player when there is support for it
