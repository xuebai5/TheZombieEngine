...
{
    //prevent exceeding the rgb buffer
    int y_height = n_min(yuv.y_height, (int)this->ti.frame_height);
    int rgbIndex=0;
    int y;
...
nOggTheoraPlayer::DecodeNextFrame()
{
    ...
    frameNr++;
    this->decodedFrames++;
    if (this->doTextureUpdate && this->GetTexture())
    {
        //this->UpdateTexture();
    }
    return;
...
nOggTheoraPlayer::Decode(nTime deltaTime)
{
    ...
    uint framesToDo = (neededFrame >= this->decodedFrames) ? (neededFrame - this->decodedFrames) : 0;
    NLOG(video, (0, "nOggTheoraPlayer::Decode(deltaTime: %f, framesToDo: %d)", float(deltaTime), framesToDo))
