...
nShadowCaster2::UpdateFaceNormalsAndMidpoints(nMesh2* mesh)
{
    ...
    const nMeshGroup& meshGroup = mesh->Group(this->meshGroupIndex);
    ushort startIndex = static_cast<ushort>(meshGroup.GetFirstIndex());
...
nShadowCaster2::WriteCapIndices(bool lit)
{
    ...
    ushort startIndex = static_cast<ushort>(this->srcMesh->Group(this->meshGroupIndex).GetFirstIndex());
...
nShadowCaster2::WriteSideIndices()
{
    ...
    ushort startEdge = static_cast<ushort>(meshGroup.GetFirstEdge());
    ushort numEdges  = static_cast<ushort>(meshGroup.GetNumEdges());
    ushort startFace = static_cast<ushort>(meshGroup.GetFirstIndex() / 3);
    ushort edgeIndex;
...
nShadowCaster2::EndWriteIndices()
{
    ...
    this->drawNumIndices = static_cast<ushort>(this->indexBufferCurrent - this->indexBufferStart);
