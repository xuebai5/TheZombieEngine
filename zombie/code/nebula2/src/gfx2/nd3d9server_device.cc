...
#define __NEBULA_NVIDIA_PERFHUD__
...
nD3D9Server::InitDeviceIdentifier()
{
    ...
    N_IFDEF_ASSERTS(HRESULT hr = )this->d3d9->GetAdapterIdentifier(N_D3D9_ADAPTER, 0, &identifier);
...
nD3D9Server::InitDeviceState()
{
    ...
    D3DADAPTER_IDENTIFIER9 identifier;
    n_dxverify2(
        this->d3d9->GetAdapterIdentifier(N_D3D9_ADAPTER, 0, &identifier),
         "nD3D9Server::InitDeviceState(): GetAdapterIdentifier() failed!" );
...
nD3D9Server::UpdateFeatureSet()
{
    // get d3d device caps
    n_dxverify2(
        this->d3d9->GetDeviceCaps(N_D3D9_ADAPTER, N_D3D9_DEVICETYPE, &(this->devCaps)),
        "GetDeviceCaps() in nD3D9Server::UpdateFeatureSet() failed!" );
...
nD3D9Server::DeviceOpen()
{
    ...
    UINT AdapterToUse = N_D3D9_ADAPTER;
    D3DDEVTYPE DeviceType = N_D3D9_DEVICETYPE;

#if defined(__NEBULA_NVIDIA_PERFHUD__) && defined(_DEBUG)
    // Look for 'NVIDIA NVPerfHUD' adapter
    // If it is present, override default settings
    for (UINT Adapter = 0; Adapter < this->d3d9->GetAdapterCount() ; Adapter++)
    {
        D3DADAPTER_IDENTIFIER9 Identifier;
        this->d3d9->GetAdapterIdentifier(Adapter, 0, &Identifier);
        if (strcmp(Identifier. Description, "NVIDIA NVPerfHUD")==0)
        {
            //MessageBox(0, Identifier. Description, Identifier. Description, 0);
    ...
    // create d3d device
    hr = this->d3d9->CreateDevice(AdapterToUse,
                                  DeviceType,
    ...
    // create effect state manager
    this->effectStateManager = nD3D9EffectStateManager::Create(this->d3d9Device);
    n_assert(this->effectStateManager);
...
nD3D9Server::DeviceClose()
{
    ...
    // destroy the effect state manager
    n_delete(this->effectStateManager);
    this->effectStateManager = 0;
...
nD3D9Server::QueryStatistics()
{
    ...
    this->dbgQueryNumVertexDeclarationChanges->SetI(this->statsNumVertexDeclarationChanges);
    this->dbgQueryNumMeshChanges->SetI(this->statsNumMeshChanges);
    this->dbgQueryNumRenderStateChanges->SetI(this->statsNumRenderStateChanges);
    this->dbgQueryNumShaderChanges->SetI(this->statsNumShaderChanges);
    ...
    this->statsNumVertexDeclarationChanges = 0;
    this->statsNumMeshChanges = 0;
...
nD3D9Server::OnDeviceCleanup(bool shutdown)
{
    ...
    else
    {
        if (this->d3dSprite)
        {
            n_dxverify( this->d3dSprite->OnLostDevice() );
        }
    }
    ...
    for (int index = 0; index < this->vertexDeclarationCache.Size(); ++index)
    {
        IDirect3DVertexDeclaration9* d3d9vdecl = this->vertexDeclarationCache.GetElementAt(index);
        d3d9vdecl->Release();
    }
    ...
    n_dxverify2(
        this->d3dxLine->OnLostDevice(),
        "OnLostDevice() on d3dxLine failed" );

    #ifdef __NEBULA_STATS__
    // release the d3d query objects
...
nD3D9Server::OnDeviceInit(bool startup)
{
    ...
    // create a query object for end command queries
    hr = this->d3d9Device->CreateQuery(D3DQUERYTYPE_EVENT, &(this->queryEvent));
    n_dxtrace(hr, "CreateQuery() failed.");
    #endif
    ...
    {
        n_dxverify( this->d3dSprite->OnResetDevice() );
    }
    ...
    // reset current vertex declaration to force setting one for the first mesh
    // (not likely to be the same as the current one, but could happen)
    this->curVertexComponents = 0;

    // send signal indicating signal restore
    this->SignalRestoreOnLost(this);    
}
...
nD3D9Server::SetSoftwareVertexProcessing(bool b)
{
    ...
    {
        n_dxverify( this->d3d9Device->SetSoftwareVertexProcessing((BOOL)b) );
    }
}
...
nD3D9Server::UpdateScissorRect()
{
    ...
    N_IFDEF_ASSERTS(HRESULT hr = )this->d3d9Device->SetScissorRect(&rect);
...
void nD3D9Server::AdjustGamma()
{
    ...
    for (int i=0; i<256; i++)
    {
        ...
        ramp.red[i] = ramp.green[i] = ramp.blue[i] = static_cast<WORD>( n_iclamp(val, 0, 65535) );
    }
...
void nD3D9Server::RestoreGamma()
{
    ...
    for (int i=0; i<256; i++)
        ramp.red[i] = ramp.green[i] = ramp.blue[i] = static_cast<WORD>( i << 8 );
...
void nD3D9Server::CreateDisplayModeEnvVars()
{
    ...
    {
        nRoot* cwd = static_cast<nRoot *> (kernelServer->GetCwd());
