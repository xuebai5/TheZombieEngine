...
*/
NSCRIPT_INITCMDS_BEGIN(nGfxServer2)
    ...
    cl->BeginSignals(1);
    N_INITCMDS_ADDSIGNAL( RestoreOnLost );
    cl->EndSignals();
...
n_setdisplaymode(void* slf, nCmd* cmd)
{
    ...
    ushort x = static_cast<ushort>( cmd->In()->GetI() );
    ushort y = static_cast<ushort>( cmd->In()->GetI() );
    ushort w = static_cast<ushort>( cmd->In()->GetI() );
    ushort h = static_cast<ushort>( cmd->In()->GetI() );
