...
nD3D9Server::nD3D9Server() :
    ...
    dbgQueryNumVertexDeclarationChanges("gfxNumVertexDeclarationChanges", nArg::Int),
    dbgQueryNumMeshChanges("gfxNumMeshChanges", nArg::Int),
    dbgQueryNumShaderChanges("gfxNumShaderChanges", nArg::Int),
    dbgQueryNumTextureChanges("gfxNumTextureChanges", nArg::Int),
    profDraw("profGFXDraw", true),
    statsFrameCount(0),
    statsNumShaderChanges(0),
    statsNumTextureChanges(0),
    statsNumRenderStateChanges(0),
    statsNumVertexDeclarationChanges(0),
    statsNumMeshChanges(0),
    statsNumDrawCalls(0),
    ...
    vertexDeclarationCache(16, 16),
    curVertexComponents(0),
    effectStateManager(0)
    #ifndef NGAME
   ,drawEnabled(true),
    drawLines(true),
    drawText(true)
    #endif
{
    // allow creating custom window handler
    windowHandler = this->CreateWindowHandler();
    ...
    // mark all shared parameters as unassigned
    int i;
    for (i = 0; i < NumTransformTypes; i++)
    {
        this->updSharedTransform[i] = true;
    }
...
nD3D9Server::OpenDisplay()
{
    ...
    if (this->DeviceOpen())
    {
        ...
        this->shapeServer.Open();
        this->shapeServer.SetShader(this->refShapeShader.get());
...
//-----------------------------------------------------------------------------
/**
    Create screen shot and save it to given filename
...
nD3D9Server::SaveScreenshot(const char* fileName)
{
    ...
    nString mangledPath = nFileServer2::Instance()->ManglePath(fileName, false);
    ...
    // save image
    D3DXIMAGE_FILEFORMAT format = this->GetFormatFromExtension(mangledPath);
    hr = D3DXSaveSurfaceToFile(mangledPath.Get(), format, this->captureSurface, NULL, NULL);
    n_assert(SUCCEEDED(hr));
...
nD3D9Server::LeaveDialogBoxMode()
{
    ...
    n_dxverify2(
        this->d3d9Device->SetDialogBoxMode(FALSE),
        "nD3D9Server::LeaveDialogBoxMode()" );
}
...
*/
nD3D9WindowHandler*
nD3D9Server::CreateWindowHandler()
{
    return n_new(nD3D9WindowHandler)(this);
}
...
*/
void 
nD3D9Server::SetCursorPosition( int x, int y )
{
    if( this->isFocus )
    {
        SetCursorPos( x, y );
...
nD3D9Server::GetFormatFromExtension(const nString& filename)
{
    ...
    else if (filename.CheckExtension("ppm"))
    {
        format = D3DXIFF_PPM;
    }
