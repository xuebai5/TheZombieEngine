...
nD3D9Font::LoadResource()
{
    ...
    if (this->fontDesc.GetFilename())
    {
        ...
        n_verify( AddFontResource(mangledPath.Get()) > 0 );
    }
    ...
    n_dxverify2(
        D3DXCreateFont(this->refD3D9Server->d3d9Device,
            height, 0,
            fontFlags, 0,
    ...
    n_dxverify2( this->d3dFont->PreloadCharacters(0, 255), "PreloadCharacters() failed");
...
nD3D9Font::UnloadResource()
{
    ...
    {
        nString mangledPath = nFileServer2::Instance()->ManglePath(this->fontDesc.GetFilename());
        n_verify( RemoveFontResource(mangledPath.Get()) != 0 );
    }
