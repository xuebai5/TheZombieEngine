...
#define N_INITCMDS_ADDSIGNAL(MemberName) {                                      \
    nSignal * signal = n_new(ScriptClass_::TSignal ## MemberName(#MemberName)); \
    bool ret = cl->AddSignal(signal);                                           \
    n_assert2(ret, "Error adding signal" # MemberName);                         \
    if (!ret)                                                                   \
    {                                                                           \
        n_delete(signal);                                                       \
    }                                                                           \
...
public:
    ...
    // my own type
    typedef nSignalNative<TR,TListIn,TListOut,signal4cc> MySignalNativeType;

    // signature information type
    typedef nCmdProtoTraits<TR,TListIn,TListOut> Traits;

    // shortcut for native binding for this signal
    template <class TClass, class TRefClass>
    struct TBindingNative
    {
        typedef nSignalBindingNative<TClass, TRefClass, TR, TListIn, TListOut> Type;
    };

    /// constructor
    nSignalNative(const char * signalName);
    /// copy constructor
    nSignalNative(const MySignalNativeType & rhs);
    /// destructor
...
template <class TR, class TListIn, class TListOut, int signal4cc>
inline
nSignalNative<TR,TListIn,TListOut,signal4cc>::nSignalNative(const MySignalNativeType & rhs)
    : nSignal(rhs)
{
    /// empty
}
...
inline
nCmdProto * 
nSignalNative<TR,TListIn,TListOut,signal4cc>::Clone() const
{
    return n_new(MySignalNativeType(*this));
}
