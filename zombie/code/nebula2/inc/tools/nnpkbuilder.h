...
public:
    ...
    /// add a mask filter to packed files
    void AddMaskFilter(const nString& mask);
...
private:
    ...
    /// check if the name is admited
    bool CheckName( const nString & name )const;

    nArray<nString> maskFilters;
    nNpkToc* tocObject;
    nFile* npkFile;
    int fileOffset;
    int dataBlockStart;
