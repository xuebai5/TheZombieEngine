...
public:
    ...
    public:
        ...
        /// set handedness
        void SetHandedness(const float& v);
        /// get handedness
        const float& GetHandedness() const;
        /// set color
        ...
        /// compare method, return -1, 0 or +1
        int Compare(const Vertex& rhs, int mask) const;
        /// equality operator
    ...
    public:
        ...
        enum Component
        {
            ...
            BINORMAL         = (1<<5),
            HANDEDNESS       = (1<<6),
            WEIGHTS          = (1<<7), // The weight in the baricentric
            JINDICES         = (1<<8), 
        };
    ...
    nMeshBuilder();
    /// constructor
    nMeshBuilder(int vertex,int triangles, int edge = 1<<15);
    /// destructor
    ~nMeshBuilder();
    ...
    /// add a triangle
    Triangle &AddTriangle(const Vertex& v0,Vertex& v1,Vertex& v2);
    /// get number of triangles
    int GetNumTriangles() const;
    ...
    /// remove vertices not referenced
    bool CleanVertex( int maxNumvertex = 65535);
    ...
    void CreateEdges();
    /// insert invisible quads for GPU shadow volume extrusion (same as with edges)
    int CreateEdgeQuads();
    /// insert invisible quads for GPU shadow volume extrusion (same as with edges)
    int CreateEdgeQuads2(const bool normalsWeightsByFace);
    ...
    /// compute the bounding box of the mesh, filtered by a group id, for billboards
    bbox3 GetGroupBBoxTestBillboard(int groupId) const;
    ...
    /// calculate the triangle weight by face
    void BuildTriangleWeights();
    /// create face normals and tangents (requires a valid uv-mapping at layer 0)
    void BuildTriangleNormals(int layer);
    ...
    /// transform all polygon in billboard format
    void TransformBillBoard();
    /// transform all normal same a sphere
    void TransformSphericalNormals();
    /// generate planes for triangles
    void BuildTrianglePlane(nArray<plane>& planeArray) const;
    /// create tangents and binormals (requires a valid uv-mapping at layer 0)
    void BuildTriangleOnlyTangentBinormal(const int layer=0);
    /// Test triangle intersection
    bool IntersectsTriangle( vector3 const& orig, vector3 const& dir, int id, float& dist) const;
    ...
    /// Test whether a given infinite vector starting from a point intersects the interior of the mesh
    bool TouchesInside(const vector3 &initPoint, const vector3 &direction);
    /// Test its contains
    bool Contains( vector3 const& pos) const;
    /// Set the terrain Color factor ( uv1.x);
    void SetTerrainColorFactor(float factor);
...
private:
    ...
    struct QuadEdge // used for generating invisible quads for shadows
    {
        int vIndex[2];
        int fIndex;
        int GroupID;
    ...
    class Quad // Use for billboard
    {
    public:
        void set (const Triangle& t1,const Triangle& t2,nMeshBuilder* mesh);
    ...
    struct PosNormalUV  // use for GetMapVertexOfSamePosNormalsUV
    {
        int idx;
        vector3 pos;
        vector3 normal;
        vector2 uv;
        float hand;
        static int __cdecl compare(const PosNormalUV& i0 , const PosNormalUV& i1);
        static int __cdecl vertexSorter(const void* elm0, const void* elm1);
    };

    
    struct SkinIndexAndWeight
    {
        SkinIndexAndWeight();
        SkinIndexAndWeight(int idx, float weight);
        ~SkinIndexAndWeight();
        int idx;
        float weight;
        static int __cdecl SorterByWeight(const void* elm0, const void* elm1);
        static int __cdecl SorterByIndex(const void* elm0, const void* elm1);
    };
    ...
    /// qsort hook for sorting quad edges for shadow volumes
    static int __cdecl QuadEdgeSorter(const void* e0, const void* e1);
    /// qsort hook for sorting  float or distances
    static int __cdecl FloatSorter(const void* elm0, const void* elm1);
    ...
    /// create a index map of vertex with the same position, normal and  uv
    int GetMapVertexOfSamePosNormalsUV(const int layer, nArray< int >& vertexToUniqueVertex);
    ...
    /// Put the handedness to vertex and create new if is necesary
    void BuildVertexHandedness();
...
inline
void
nMeshBuilder::Vertex::SetHandedness(const float& v)
{
    this->handedness = v;
    //this->compMask |= HANDEDNESS;
}
...
inline
const float&
nMeshBuilder::Vertex::GetHandedness() const
{
...
inline
void
nMeshBuilder::Triangle::SetHandedness(const float& v)
{
    this->handedness = v;
    this->compMask |= HANDEDNESS;
}
...
inline
const float&
nMeshBuilder::Triangle::GetHandedness() const
{
    n_assert( HANDEDNESS & this->compMask);
    return this->handedness;
}
//------------------------------------------------------------------------------
/**
...
inline
void
nMeshBuilder::Triangle::SetWeights(const vector4& w)
{
    this->weights = w;
    this->compMask |= WEIGHTS;
}
...
inline
const vector4&
nMeshBuilder::Triangle::GetWeights() const
{
    n_assert(WEIGHTS & this->compMask);
    return this->weights;
}
...
inline
void
nMeshBuilder::Triangle::SetJointIndices(const vector4& ji)
{
    this->jointIndices = ji;
    this->compMask |= JINDICES;
}
...
inline
const vector4&
nMeshBuilder::Triangle::GetJointIndices() const
{
    n_assert(JINDICES & this->compMask);
    return this->jointIndices;
}
