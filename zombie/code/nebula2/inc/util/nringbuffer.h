...
public:
    ...
    // clear ring buffer (does not deallocate anything)
    void Clear();
...
//------------------------------------------------------------------------------
/**
    MakeOldest makes the element the oldest, which is done through swapping
    the contents with the tail.
*/
template<class TYPE>
void 
nRingBuffer<TYPE>::Clear()
{
    n_assert(this->start);
    this->tail = this->head;
}
