...
public:
    ...
    /// Sort
    void QSort( int (__cdecl *compare )(const void *, const void *) );
...
nArray<TYPE>::Erase(typename nArray<TYPE>::iterator iter)
{
    ...
    n_assert( (iter - this->elements) <= INT_MAX );
    this->Erase( static_cast<int>( iter - this->elements ) );
    return iter;
}
...
{
    n_assert(this->elements && (iter >= this->elements) && (iter < (this->elements + this->numElements)));
    this->EraseQuick( static_cast<int>( iter - this->elements ) );
    return iter;
}
...
/**
    Sort the array
    @param  compare  function for compare.
*/
template<class TYPE>
void
nArray<TYPE>::QSort( int (__cdecl *compare )(const void *, const void *) )
{
    if ( this->Size() > 0 )
    {
       qsort( this->elements, this->Size() , sizeof(TYPE) , compare);
    }
}
