...
public:
    enum 
    {
        /// log group used for asynchronous input/output operations
        NLOG_ASYNCIO  = NLOG1,
    };
    ...
    /// enable/disable automatic reload on change behavior
    void SetReloadOnChange(bool b);
    /// get reload on change behaviour
...
//------------------------------------------------------------------------------
/**
    Set behavior to reload on change. Used mainly by directory watcher to avoid
    reload resources that have changed in Nebula and not by external 
    applications. It is enabled by default, disable it in resource modified in 
    Nebula. And do it every time the resource is modified, in this way we could 
    even detect when a modification has been done internally or externally for 
    the same resource (since the directory watcher reset after getting a
    modification event for the resource).
*/
inline
void 
nResource::SetReloadOnChange(bool b)
{
    this->reloadOnChange = b;
}
...
inline
bool 
nResource::GetReloadOnChange() const
{
