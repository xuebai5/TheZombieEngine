...
/**
    ...
    @section InputLogToFile Input Logging to a file and Replay of events

    There are functions to log the events in the input server. But the log to a 
    file allow also to replay the same input events later. To allow the replay the 
    file save also the time stamp so the the replay will be in the same tempo.

    The time stamp is relative to the begin of the logging.

    Functions to log to a file and from a file: 
        StartLogToFile, StartLogFromFile, StopLogFile, IsLogFileRunning
*/
...
public:
    ...
    /// get mute mode
    bool GetMute() const;
    /// get the state of the ctrl, alt, shit buttons
    int GetQualifierFlags()const;
    /// set the state of some qualifier button
    void SetQualifierFlag( nKey key );
    /// unset the state of some qualifier button
    void UnsetQualifierFlag( nKey key );
    /// reset the actual state of qualifier keys
    void ResetQualifierFlag();

    /// log the input to a file
    void StartLogToFile(nString *);
    /// load input from a file
    void StartLogFromFile(nString *);
    /// stop log to/from file
    void StopLogFile ();
    /// say if log file is running
    bool IsLogFileRunning ();
...
protected:
    ...
    /// log all events
    void LogEvents();
    /// log events to a file or get events from file
    void LogFileEvents();
    /// get events from a log file
    void LogFileEventsFromFile();
    /// log events to a file
    void LogFileEventsToFile();
    /// log file clean
    void LogFileClean();
    /// get a number of events from a file
    void LogFileGetEvents( const int num );
    /// get the string with the name of a nInputType
    const char * InputTypeToString( const nInputType type );
    /// get a nInputType from a name
    const nInputType StringToInputType( const nString &name );
...
protected:
    ...
    double  log_init_time;
    logType log_file;
    nList   log_file_events;
    nList   log_file_oldEvents;
    nFile * logFileObject;
...
inline
int
nInputServer::GetQualifierFlags() const
{
    return this->qualifierFlags;
}
