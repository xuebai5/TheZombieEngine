...
public:
    ...
    /// set rect using x,y, width, height in parent widget's space
    void SetRectWH(const rectangle& r);
...
inline
void
nGuiWidget::SetRectWH(const rectangle& r)
{
    rectangle new_r( vector2(r.v0.x, r.v0.y), 
                     vector2(r.v0.x + r.v1.x, r.v0.y + r.v1.y) );

    this->OnRectChange(new_r);
}
