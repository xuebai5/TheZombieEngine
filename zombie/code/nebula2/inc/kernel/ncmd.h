...
public:
    ...
    /// Copy input arguments from an argument list
    void CopyInArgsFrom(int numargs, nArg * arglist);
...
inline
void
nCmd::CopyInArgsFrom(int numargs, nArg * arglist)
{
    n_assert(arglist || numargs == 0);
    n_assert(numargs == this->cmdProto->GetNumInArgs());

    // rewind command
    this->Rewind();

    // copy the arguments from the argument list provided 
    for(int i = 0;i < numargs;i++)
    {
        nArg * argSrc = &arglist[i];
        nArg * argDst = this->In();
        n_assert(argSrc->GetType() == argDst->GetType());
        argDst->Copy(*argSrc);
    }
