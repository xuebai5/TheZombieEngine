...
public:
    ...
    void SetO(void*);
    /// set content to a memory pointer (only runtime, don't save)
    void SetP(void*);
...
nArg::Copy(va_list * marker)
{
    switch(this->type) 
    {
        ...
        case Char:
            // chawr is promoted to int when passed through
            {
                int tmp = va_arg(*marker, int);
                this->c = (char)tmp;
            }
...
/**
    ...
    @param _c the char value
*/
inline 
void 
nArg::SetC(char _c)
{
    n_assert((Void == this->type) || (Char == this->type));
    this->type = Char;
    this->c = _c;
}
...
//-----------------------------------------------------------------------------
/**
    Sets the contents to a memory pointer, and sets the arg type to
    Pointer. The pointer is ONLY FOR RUNTIME, NOT SAVE.

    @param _o the memory pointer
*/
inline 
void 
nArg::SetP(void *_o)
{
    n_assert((Void == this->type) || (Pointer == this->type));
    this->type = Pointer;
    this->o = _o;
}
...
//-----------------------------------------------------------------------------
/**
Set the content to a float3 value.
*/
inline
void
nArg::SetF3 (const nFloat3& _f3)
{
    n_assert((Void == this->type) || (Float3 == this->type));
    this->type = Float3;
    this->f3 = _f3;
}
...
//-----------------------------------------------------------------------------
/**
    Returns the char content of the object. Throws assertion if
    not set to a char.
...
//-----------------------------------------------------------------------------
/**
    Returns the memory pointer content of the object. Throws assertion if
    not set to an memory pointer.
...
//-----------------------------------------------------------------------------
/**
    Return float3 content.
*/
inline
const nFloat3&
nArg::GetF3() const
{
    n_assert(Float3 == this->type);
    return this->f3;
}
...
nArg::IsValidArg(char c)
{
    switch (c)
    {
        ...
        case 'l':
            return true;
        case 'p': /*Not USE in scripting, only C++ Code*/
            n_assert_always();
            return false;
        default:
            return false;
