...
public:
    ...
    /// find a loaded class object by name, and load it if not loaded
    nClass* FindClass(const char* className);
    /// find a loaded class object by name, return 0 if it is not loaded
    nClass* IsClassLoaded(const char* className);
    /// return the list of classes
    const nHashList * GetClassList() const;

    /// create a Nebula object, fail hard if no object could be created
    nRoot* New(const char* className, const char* objectName, bool initInstance = true);
    /// create a Nebula object, fail hard if no object could be created
    nObject* New(const char* className, bool initInstance = true);
    /// create a Nebula object, don't fail when no object could be created
    nRoot* NewNoFail(const char* className, const char* objectName, bool initInstance = true);
    /// create a Nebula object, don't fail when no object could be created
    nObject* NewNoFail(const char* className, bool initInstance = true);
    /// create a Nebula object from a persistent object file
    nObject* Load(const char* path, bool initInstance = true);
    /// create a Nebula object from a persistent object file with given name
    nRoot* LoadAs(const char* path, const char* objName, bool initInstance = true);
    /// create a Nebula object from a persistent object file
    nObject* Load(nFile* file, bool initInstance = true);
    /// create a Nebula object from a persistent object file with given name
    nRoot* LoadAs(nFile* file, const char* objName, bool initInstance = true);
    /// lookup a Nebula object in the hierarchy
    nRoot* Lookup(const char* path);

    /// set current working object
    void SetCwd(nObject* newCwd);
    /// get current working object
    nObject* GetCwd();
    /// push current working object on stack, and set new cwd
    void PushCwd(nObject* newCwd);
    ...
    void AddModule(const char *name, nClass * (*_init_func)(const char *, nKernelServer *));
    ...
    void Trigger(void);
    /// 
    typedef void (__cdecl *CallBack)(const char* className);
    /// Set the callback. this is called when class is not found
    void SetCallBackClassNotFound( CallBack callBackClassNotFound);
    /// Get the callback
    CallBack  GetCallBackClassNotFound();
...
private:
    ...
    nObject* cwd;                   // the current working object
    nStack<nObject*> cwdStack;      // stack of previous cwd's
    ...
    /// This method is called when class not found
    CallBack callBackClassNotFound;
};
