...
public:
    ...
    /// check if sphere is contained in bbox
    bool isContainedIn(const bbox3& box) const;
...
inline
bool
sphere::contains(const vector3 &point) const
{
    vector3 v(point - this->p);
    if (v.lensquared() <= r*r)
    {
        return true;
    }
...
//------------------------------------------------------------------------------
/**
    return true if this box fully contains the sphere ---
*/
inline
bool
sphere::isContainedIn(const bbox3& box) const
{
    // check if the sphere exceeds the limits of the box
    if ((box.vmin.x < (this->p.x - this->r)) && (box.vmax.x >= (this->p.x + this->r)) &&
        (box.vmin.y < (this->p.y - this->r)) && (box.vmax.y >= (this->p.y + this->r)) &&
        (box.vmin.z < (this->p.z - this->r)) && (box.vmax.z >= (this->p.z + this->r)))
    {
        return true;
    }
