...
public:
    ...
    //not usable until precision corrections antonia.tugores
    ...
    void Set(const matrix33& m, int order)
    {
        int i,j,k,h,n,s,f;
        ...
        if (s == EulRepYes)
        {
            ...
            {
                this->x = (float) atan2f(m.m[0][1], m.m[0][2]);
                this->y = (float) atan2f(sy, m.m[0][0]);
                this->z = (float) atan2f(m.m[1][0], -m.m[2][0]);
            }
            else
            {
                this->x = (float) atan2f(-m.m[1][2], m.m[1][1]);
                this->y = (float) atan2f(sy, m.m[0][0]);
                this->z = 0;
            }
        }
        else
        {
            ...
            {
                this->x = (float) atan2f(m.m[2][1], m.m[2][2]);
                this->y = (float) atan2f(-m.m[2][0], cy);
                this->z = (float) atan2f(m.m[1][0], m.m[0][0]);
            }
            else
            {
                this->x = (float) atan2f(-m.m[1][2], m.m[1][1]);
    ...
    /// set with default nebula euler
    void Set(const matrix33& m)
    {
        this->Set(m, EulOrdXYZs);
    }
    //-------------------------------------------------------------------------------- 
    /**
