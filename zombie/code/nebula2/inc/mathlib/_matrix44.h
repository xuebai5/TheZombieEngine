...
public:
	...
	/// transform vector3, only use matrix 3x3
	_vector3 transform_norm(const _vector3& v) const;
...
//------------------------------------------------------------------------------
/**
    Transforms a vector by the  3x3 matrix
*/
inline
_vector3
_matrix44::transform_norm(const _vector3& v) const
{
    return _vector3(
...
inline
void 
_matrix44::get(_vector3& scale,quaternion& quat,_vector3& pos) const
{
    float sx, sy, sz;
    ...
    tmp.scale( vector3(1.f / sx , 1.f/sy, 1.f/sz) );
    tmp=tmp*(*this);
