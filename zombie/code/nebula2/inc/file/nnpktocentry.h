...
nNpkTocEntry::~nNpkTocEntry()
{
    if (this->entryList)
    {
        ...
        while ( 0 != (curEntry = (nNpkTocEntry*) this->entryList->RemHead()))
        {
            n_delete(curEntry);
        }
...
inline
const char*
nNpkTocEntry::GetFullName(char* buf, int N_IFDEF_ASSERTS(bufSize) )
{
    ...
    if (this->rootPath)
    {
        ...
        n_assert( strlen( this->rootPath ) < N_MAXPATH );
        curBufIndex = static_cast<int>( strlen(this->rootPath) );
        buf[curBufIndex++] = '/';
    }
