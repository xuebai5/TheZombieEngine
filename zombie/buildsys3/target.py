...
class Target:
    ...
    #--------------------------------------------------------------------------
    def GetWarningsLevel (self, platform):         
        if 'win32' == platform:
            if '' != self.warningsLevel:                       
                if self.warningsLevel in ("0","1","2","3","4"):
                    return self.warningsLevel
                else:
                    self.buildSys.logger.warning('Invalid warning level' \
                                               ' \"%s\" for target %s in %s' \
                                               '. Continuing using default'\
                                               ' (default = 3)', 
                                               self.warningsLevel, self.name, 
                                               self.bldFile)
                    return "4"
            else:
                # If no command used or value not given, return the default
                return "4"
        else:
            #not implemented
            return 0
    ...
    #--------------------------------------------------------------------------
    # Scan the modules and adds their submodules
    def ProcessSubmodules(self):
        for moduleName in self.modules:
            curModule = self.buildSys.modules[moduleName]
            for submod in curModule.submodules :
                self.modules.append( submod.name )
