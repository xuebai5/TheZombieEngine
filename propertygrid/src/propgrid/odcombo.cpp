...
{
public:
    wxComboFrameEventHandler( wxCustomComboBox* pCb );
...
wxComboFrameEventHandler::wxComboFrameEventHandler( wxCustomComboBox* combo )
    : wxEvtHandler()
{
    m_combo = combo;
}
...
public:
    ...
    wxComboBoxExtraInputHandler( wxCustomComboBox* combo )
        : wxEvtHandler()
    {
        m_combo = combo;
    }
...
IMPLEMENT_DYNAMIC_CLASS(wxCustomComboBox, wxCustomComboBoxBase)

BEGIN_EVENT_TABLE(wxCustomComboBox, wxCustomComboBoxBase)
    EVT_MOUSE_EVENTS(wxCustomComboBox::OnMouseEvent)
    EVT_PAINT(wxCustomComboBox::OnPaint)
    EVT_SIZE(wxCustomComboBox::OnResize)
    //EVT_KEY_DOWN(wxCustomComboBox::OnKey)
    //EVT_KEY_UP(wxCustomComboBox::OnKey)
    //EVT_SIZE(wxCustomComboBox::OnResize)
    EVT_SET_FOCUS(wxCustomComboBox::OnFocusEvent)
    EVT_KILL_FOCUS(wxCustomComboBox::OnFocusEvent)
END_EVENT_TABLE()


void wxCustomComboBox::Init()
{
...
bool wxCustomComboBox::Create(wxWindow *parent,
                              wxWindowID id,
...
wxCustomComboBox::~wxCustomComboBox()
{
#if wxODC_INSTALL_TOPLEV_HANDLER
...
wxSize wxCustomComboBox::DoGetBestSize() const
{
    wxASSERT ( m_btn );
...
void wxCustomComboBox::DoMoveWindow(int x, int y, int width, int height)
{
...
bool wxCustomComboBox::Enable(bool enable)
{
    if ( !wxCustomComboBoxBase::Enable(enable) )
        return false;
...
bool wxCustomComboBox::Show(bool show)
{
    if ( !wxCustomComboBoxBase::Show(show) )
        return false;
...
bool wxCustomComboBox::SetFont ( const wxFont& font )
{
    if ( !wxCustomComboBoxBase::SetFont(font) )
        return false;
...
/*
void wxCustomComboBox::SetFocus()
{
    //wxCustomComboBoxBase::SetFocus();
    //wxLogDebug(wxT("wxCustomComboBox::SetFocus()"));
    if (m_text)
    {
        m_text->SetFocus();
    }
...
/*void wxCustomComboBox::SetFocusFromKbd()
{
    SetFocus();
}*/

#if wxUSE_TOOLTIPS
void wxCustomComboBox::DoSetToolTip(wxToolTip *tooltip)
{
    wxCustomComboBoxBase::DoSetToolTip(tooltip);
...
void wxCustomComboBox::SetId( wxWindowID winid )
{
    wxCustomComboBoxBase::SetId(winid);
...
void wxCustomComboBox::OnItemPaint( wxDC& dc, const wxRect& rect )
{
    if ( !m_text )
    {
...
void wxCustomComboBox::OnPaint( wxPaintEvent& )
{
    wxPaintDC dc(this);
...
void wxCustomComboBox::OnPopupDismiss()
{
...
void wxCustomComboBox::HidePopup( bool WXUNUSED(sendEvent) )
{
    wxASSERT( m_winPopup );
...
void wxCustomComboBox::SendShowPopupSignal()
{
    wxCommandEvent evt(wxEVT_COMMAND_BUTTON_CLICKED,GetId());
    GetEventHandler()->AddPendingEvent(evt);
}

void wxCustomComboBox::OnMouseEvent( wxMouseEvent& event )
{
    int type = event.GetEventType();
...
void wxCustomComboBox::OnResize( wxSizeEvent& event )
{
    if ( !m_btn )
        return;

    //wxLogDebug(wxT("wxOwnerDrawnComboBox::OnResize"));
...
void wxCustomComboBox::OnFocusEvent( wxFocusEvent& )
{
    //wxLogDebug(wxT("wxCustomComboBox::OnSetFocus()"));
...
/*void wxCustomComboBox::OnKey(wxKeyEvent& event)
{
    if ( m_isPopupShown > 0 )
    {
        // pass it to the popped up control
...
void wxCustomComboBox::SetPopupExtents( int extLeft, int extRight, int prefHeight )
{
    m_extLeft = extLeft;
    m_extRight = extRight;
...
void wxCustomComboBox::SetCustomPaintArea( int width )
{
    if ( m_text )
    {
...
wxString wxCustomComboBox::GetValue() const
{
    if ( m_text )
...
void wxCustomComboBox::SetValue(const wxString& value)
{
    if ( m_text )
...
void wxCustomComboBox::Copy()
{
    wxASSERT_MSG ( m_text, wxT("Do not call this wxCustomComboBox method in read-only mode.") );
    GetTextCtrl()->Copy();
}

void wxCustomComboBox::Cut()
{
    wxASSERT_MSG ( m_text, wxT("Do not call this wxCustomComboBox method in read-only mode.") );
    GetTextCtrl()->Cut();
}

void wxCustomComboBox::Paste()
{
    wxASSERT_MSG ( m_text, wxT("Do not call this wxCustomComboBox method in read-only mode.") );
    GetTextCtrl()->Paste();
}

void wxCustomComboBox::SetInsertionPoint(long pos)
{
    wxASSERT_MSG ( m_text, wxT("Do not call this wxCustomComboBox method in read-only mode.") );
    GetTextCtrl()->SetInsertionPoint(pos);
}

void wxCustomComboBox::SetInsertionPointEnd()
{
    wxASSERT_MSG ( m_text, wxT("Do not call this wxCustomComboBox method in read-only mode.") );
    GetTextCtrl()->SetInsertionPointEnd();
}

long wxCustomComboBox::GetInsertionPoint() const
{
    wxASSERT_MSG ( m_text, wxT("Do not call this wxCustomComboBox method in read-only mode.") );
    return GetTextCtrl()->GetInsertionPoint();
}

long wxCustomComboBox::GetLastPosition() const
{
    wxASSERT_MSG ( m_text, wxT("Do not call this wxCustomComboBox method in read-only mode.") );
    return GetTextCtrl()->GetLastPosition();
}

void wxCustomComboBox::Replace(long from, long to, const wxString& value)
{
    wxASSERT_MSG ( m_text, wxT("Do not call this wxCustomComboBox method in read-only mode.") );
    GetTextCtrl()->Replace(from, to, value);
}

void wxCustomComboBox::Remove(long from, long to)
{
    wxASSERT_MSG ( m_text, wxT("Do not call this wxCustomComboBox method in read-only mode.") );
    GetTextCtrl()->Remove(from, to);
}

void wxCustomComboBox::SetSelection(long from, long to)
{
    wxASSERT_MSG ( m_text, wxT("Do not call this wxCustomComboBox method in read-only mode.") );
    GetTextCtrl()->SetSelection(from, to);
}

/*void wxCustomComboBox::SetEditable(bool editable)
{
...
IMPLEMENT_DYNAMIC_CLASS(wxOwnerDrawnComboBox, wxCustomComboBox)


BEGIN_EVENT_TABLE(wxOwnerDrawnComboBox, wxCustomComboBox)
                            ...
                            const wxString& name)
{
    ...
    return wxCustomComboBox::Create(parent,id,value,pos,size,style,validator,name);
}
...
void wxOwnerDrawnComboBox::OnItemPaint( wxDC& dc, const wxRect& rect )
{
    ...
    if ( !(m_windowStyle & wxODCB_STD_CONTROL_PAINT) && callback )
    {
        ...
        wxCustomComboBox::OnItemPaint(dc,rect);
}
